...
> **Remediation status (2026-07-25): COMPLETE.** All seven phases of §7 landed on main
> as commits `e61f5580` (phase 1) through `78ea992c` (phase 7). See the
> **Remediation record** appendix at the end of this document for the before/after
> metrics, the intentionally-retained raw-color exemptions, and the two follow-ups
> that were split into their own tasks (Radix→sonner toast cleanup was included;
> the pre-existing mocked-e2e suite failures were not caused — or fixable — by this
...
156 arbitrary font sizes below `text-xs`: `text-[10px]` ×82, `text-[11px]` ×44, plus a parallel rem family (`text-[0.8rem]` ×9, `[0.7rem]` ×5, `[0.65rem]` ×2) spelling the _same_ visual sizes differently. Add `2xs`/`3xs` fontSize tokens and codemod.
...
## Remediation record (2026-07-25)

All phases landed on main, one commit per phase:

| Phase               | Commit     | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 Correctness       | `e61f5580` | Toast system (headless Radix store deleted, sonner everywhere, skin restored), content globs + `index.html`, `components.json` aliases, success/warning/error-foreground vars fixed, `loading` keyframe defined, NODE_TYPES `as const` + EventNode registered correctly (dead selectors removed), print CSS scoped to `body.printing-report`, Inter self-hosted via `@fontsource/inter`, theme plumbing (single owner, `color-scheme`, `theme-color` meta, pre-paint bootstrap), reactive dark reads in canvas edges/graphs                                           |
| 2 Dead weight       | `03f5b348` | error color family, custom animation block, dvh/svh/lvh + safe-area duplicates, `--accent-*`/`--border-light`, `scrollbar-gutter-stable`, sidebar-primary pair, stray sidebar var; `report-print-brand` implemented (rendered in print footer) rather than deleted                                                                                                                                                                                                                                                                                                    |
| 3 Tooling           | `bc87364d` | `prettier-plugin-tailwindcss` + one mechanical reformat; duplicate `.prettierrc.json` removed                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 4 Dark-mode detox   | `3eee73b3` | Token sweep of override-dependent files (maintenance-page, metric-card, FolderDialogs, form-section, tables/headings), then **all global `.dark` class/element overrides deleted**; desired values folded into the `.dark` variable block (`--foreground` 75%, table styling in components)                                                                                                                                                                                                                                                                           |
| 5 Shared components | `62229b7f` | Spinner/PageLoader (name collision resolved), standardized Skeleton, ConsoleListState (skeleton/error/empty trio ×8 pages), tokenized StatusPill + exported tone map, `--info` token, Alert warning/info variants (8 banners), FieldError, NativeSelect, SectionLabel (44 of ~60 labels), PageContainer, Button `icon-sm` (50 sites), picker primitives + geometry constants, MappingModeToggle, EditorTable (21 th cells), reports BlockFrame, node-dimension single source (NODE_TYPE_SIZES), `window.confirm`→ConfirmationDialog, JSX `<style>` keyframes → config |
| 6 Token migration   | `aedcd96c` | ~140 status-color sites → success/warning/destructive/info (raw 448→105, tokens 22→492), ExecutionTimeline hex map → classes, validated chart palette promoted to `--chart-1..9` with consumers wired, `text-2xs`/`text-3xs` tokens (95 sites), named `z-60` + portal alignment, CANVAS_Z ladder, `prefers-reduced-motion` support                                                                                                                                                                                                                                    |
| 7 Polish            | `78ea992c` | focus-visible idiom everywhere (0 stray `focus:ring` left), WithTooltip on 22 icon-only controls, `size-*` codemod (815 conversions), 14 static inline styles → utilities, shared dim constants + wizard grid templates, 19 raw buttons gained focus rings                                                                                                                                                                                                                                                                                                            |

**Verification per phase:** `tsc -b`, ESLint, 1105-test vitest suite, and the mocked
Playwright suite; production build re-verified at closeout. Browser-level visual
checks (light + dark) after phases 1, 4, and 6.

**Honest accounting of the e2e baseline:** the mocked Playwright suite was already
failing 34 of 69 tests on main _before_ this work (verified by checking out
`3858949c` and running the suite there). The CSS remediation introduced **zero new
failures** (failure-set diff is empty) and fixed one spec whose page-wide locator
became ambiguous because error toasts now actually render. The pre-existing 34
failures are tracked as a separate task.

**Intentionally retained raw palette (~105 class sites, documented KEEP decisions):**
NoteNode sticky-note yellows, WorkflowLogs search-highlight marks, folder-icon
ambers, AI-agent purples, ExecutionTimeline step-type identity hues, service-icon
and platform brand gradients/hexes, composite-mode editor greens, the
condition-editor value-type triad (cyan/orange/violet), and System-analytics
metric-category accents. These are identity/decorative colors, not status.

**Residual (accepted):** ~50 bespoke `animate-pulse` placeholder shapes in features
now all share the standardized `bg-muted/N` recipe (the primitive-vs-copy drift is
resolved; converting content-shaped placeholders wholesale risked visual churn for
no consistency gain). `title=` remains on truncated-text spans as a cheap overflow
affordance per the audit's own recommendation.
