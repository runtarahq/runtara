...
- **The first "Edit" click after a page load is swallowed.** Reproduced on the *Done* (Finish) step and the *AI Agent* step; the second click opens the editor. Worth a repro pass — likely a mount-order race between `editingNodeId` and `editingNodeData` (`WorkflowEditor/index.tsx:431-441`, `renderTimelineInlineEditor` at `:1877-1881` returns `null` when `editingNodeData` has not caught up).
- **I could not open the AI Agent step config at all** in *Demo AI* — Timeline "Edit AI Agent", canvas double-click and right-click were all inert, despite `AiAgentStepField` being 1207 lines and reachable at `NodeFormItem.tsx:756`. Needs a dedicated repro; if confirmed, the flagship step type is uneditable in the UI and this jumps to Wave 1.
