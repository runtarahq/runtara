...
|---|---|---|
| `oidc` (default) | RUNTARA, via JWKS | You want RUNTARA to act as a relying-party against a hosted IdP (Auth0, Okta, Entra ID, Keycloak Cloud, Zitadel Cloud, etc.). |
...
`TENANT_ID` is required in every mode — RUNTARA is single-tenant per process. Unset, the server exits with `missing required environment variable: TENANT_ID` and status `1`.
...
| Var | Required | Notes |
|---|---|---|
| `OAUTH2_JWKS_URI` | yes | RUNTARA fetches this on startup and refreshes hourly. **Unset → server exits.** Set but unreachable is *not* fatal: the server binds and reports itself unready until the keys land. |
...
| `RUNTARA_MCP_REQUIRE_AUDIENCE` | no | When set, MCP additionally rejects tokens that omit `aud` entirely. Setting it without `OAUTH2_MCP_AUDIENCE` is a boot-time failure. |
...
`RUNTARA_AUTH_REQUIRE_JTI` and `RUNTARA_AUTH_MEMBERSHIP_POLICY` are inert in `local` and `trust_proxy` modes — there is no membership store to consult, and every caller acts as the tenant Owner (see below) — so they are safe to leave set across mixed fleets. Two caveats: an *unrecognized* `RUNTARA_AUTH_MEMBERSHIP_POLICY` value panics at startup in every mode, so keep it to `disabled|logging|required`; and `RUNTARA_DEV_ROLE` (below) only has teeth when the policy is `required`.
...
| `SERVER_HOST` | must be loopback | `127.0.0.1`, `::1`, or `localhost`. Enforced at startup, and **not** the default — set it explicitly. |
...
`AUTH_PROVIDER=trust-proxy` (hyphen) is accepted as an alias.

Every proxy-authenticated caller (and every API key) acts as the tenant **Owner**: there is no per-tenant membership store in this mode, so role-based authorization is fully permissive regardless of `RUNTARA_AUTH_MEMBERSHIP_POLICY`. The "User Management" link is hidden for proxy-authenticated callers — that surface belongs to the managed control plane, which self-hosted deployments don't have. (An API-key caller is *not* classified as self-hosted, so `/me` still advertises the capability to it; the link it points at is inert.)

## `AUTH_PROVIDER=local`

No authentication at all. Every request is served as the configured tenant with `user_id = "local"` (override with `RUNTARA_DEV_USER_ID`). This is intended for:
...
❌ Configuration error: AUTH_PROVIDER=trust_proxy requires SERVER_HOST to be a
   ...
   a reverse proxy in front of it. See docs/deployment/auth-modes.md.
```
