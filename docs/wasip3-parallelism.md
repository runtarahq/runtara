...
**Status:** IN PROGRESS. Phase 0 LANDED (wasmtime 46 + Rust 1.97 `d43f556a`; cargo-component exit `42d05179` — 26 agents + stdlib/runtime on plain cargo + wit-bindgen 0.58, `direct_wasm_execute` 95/95). Phase 1 spikes ALL PASS: **S1** `82c92175` (`spikes/wasip3-stackful`) — hand-emitted stackful lift + async lowers + waitable-set loop, production 0.247 pins, composed via the production textual-wac pipeline, runs on 46 with epoch interruption: run-both ~152ms vs run-seq ~304ms (true overlap; callback fallback not needed; no wasm-tools upgrade needed). **S2** — route (b) (host-mediated `async func` import, `func_wrap_concurrent`) proven by the S1 harness; wasmtime-wasi-http 46 ships a p3 host behind its `p3` feature, so route (a) is available later. **S3** `d890279f` (`spikes/wit-bindgen-async-typed`) — wit-bindgen 0.58 `async: false` sync-implements async-typed exports (type preserved, sync lift, 1.249-valid): agent recipe = re-type WIT + one macro option, zero body changes.
...
Today a retry backoff blocks in a host sleep: the **keyed `durable-sleep-checkpoint`** on the rate-limited path (`agent_retry.rs:114-159` retry-after branch; also Delay `delay.rs:148`, Split retry `split_retry.rs:194`, Embed retry `embed_retry.rs:198`) and plain `durable-sleep` otherwise — inside a single stackful task either would stall every sibling. With both sleeps re-typed `async func` and lowered async, **a sleep is just another waitable** in the same set. Per-item retry state lives in the call slot; the completion loop treats a timer's RETURNED event as "relaunch attempt n+1 for slot i". The per-attempt durability contract is unchanged: `::attempt::N` err-only checkpoints, backoff gated on the checkpoint-hit flag exactly as the sequential lowering does (unify Blocker-B semantics).
...
### 4.3 Signals and cancellation
Today these polls exist only at While/Wait/Embed loop boundaries (`while_loop.rs:326,440`, `wait.rs:127,428`, `embed_workflow.rs:805`) — the sequential Split loop does **not** poll mid-loop. The parallel Split's completion loop therefore *adds* `check-signals`/`is-cancelled` at event-wakeup granularity (a strict improvement: today a long Split is uninterruptible between iterations' host calls). Same rate-limited host calls (1s server-side limiter), bounded staleness of one event; both re-typed `async func` so a poll never freezes in-flight siblings (§3.1).
...
> **Drain-then-suspend at window granularity:** stop launching new items; keep consuming completion events and `persist()`-ing each item that lands; **items parked on a retry-backoff timer are not waited out** — their settled state is already durable at `::attempt::N`, so their timers are dropped and their backoff deadlines are folded into the exit wake-set as `at(deadline)` entries (the environment collapses multiple `at`s to min → `sleep_until`, per the unify contract); when no agent subtask remains in flight, exit with `outcome::suspended(wake-set)` (for a plain drain with nothing pending: `on-resume`).
