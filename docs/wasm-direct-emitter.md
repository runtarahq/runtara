...
- The direct core emitter now has the first static `EmbedWorkflow` lowering
  ...
  isolation/cache-key behavior, deeply nested child failure wrapping,
  child-local `onError` handling for nested static `EmbedWorkflow` failures, and
  parent `EmbedWorkflow.onError` handling after child retry exhaustion.
  Retry/backoff support includes default policy materialization, explicit
  `maxRetries` / `retryDelay`, retry-attempt recording, and nested parent-frame
...
- `tests/direct_wasm_ab_execute.rs` now provides the first strict direct-vs-Rust
  ...
  child-local `onError` handling for a nested static child failure, nested
  parent retry through a failing grandchild, and deeply nested child failure
  wrapping, with preloaded child graphs passed through both compile paths.
...
- Durable Split breakpoints are now supported. Direct mode emits the
  generated-compatible breakpoint checkpoint/event before Split cache-key
  lookup, iteration count resolution, debug-start emission, and nested body
  execution, using the existing shared breakpoint runtime ABI and stdlib event
  helper. The stdlib now builds generated-compatible Split breakpoint inputs
  from the Split config (`value`, `parallelism`, `sequential`,
  `dontStopOnFailed`, `allowNull`, `convertSingleValue`, `batchSize`, and
  optional `variables`). Support tests remove the previous `split-breakpoint`
