...
| 4 — Single render pipeline | [~] In progress. Carry-forwards landed: virtual aggregate engine moved to `runtara-report-dsl::virtual_aggregate` (feature-gated, reuses `runtara-object-store` canonical types); `aggregate_with_optional_joins` + `resolve_join` folded into `ObjectModelProvider`; `render_aggregate_table_block` / `render_aggregate_block` now uniformly dispatch through `provider.fetch_aggregate`. Remaining: `render_blocks` consolidation, `Renderer` trait per block type, page/offset/sort helper collapse, `render_metric_block` typed `AggregateResult`. |
...
**Status:** [~] In progress. Phase 3's two carry-forwards landed.
