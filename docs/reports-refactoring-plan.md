...
**Status:** Phase 1 complete; Phase 2 complete (codegen + types.ts deletion + utils.ts template/row-condition WASM swap + lazy-load wizard route). Deferred follow-ups: row-condition editor/wire-format migration to canonical `ConditionExpression`, schemars 0.8/1 consolidation — both require focused dedicated commits, see progress log.
**Owner:** _unassigned_
