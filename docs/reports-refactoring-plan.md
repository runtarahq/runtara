...
| 7 — Wizard rewrite | [~] In progress. New `components/wizard-v2/` tree (~1.6k LOC) operates on `ReportDefinition` directly behind `?wizard=v2`. Block list + per-type editors (markdown / table / metric / chart / card / actions), filters/datasets/views editors, explicit "Reset to dataset schema" diff dialog, shared workflow `ConditionEditor` for visibility predicates. Server-side `normalize_definition_connections` injects the default object-model connection on get/create/update/preview/validate. Lossless round-trip on all 11 corpus fixtures + 11 layoutOps unit tests + 4 block-editor unit tests pass. Old wizard coexists; deeper feature parity (dataset dimension/measure editor, advanced card subgrids, full filter options config) still routes to legacy and is the remaining v2 work before Phase 8 can retire the legacy wizard. |
...
**Status:** [~] In progress. Wizard v2 ships behind `?wizard=v2`; deeper feature parity (full dataset dim/measure editor, advanced card subgrids, full filter options config) is the remaining v2 work and currently routes to legacy. Old wizard intentionally coexists for one release per the original plan.

### Work

- [x] Wizard operates on `ReportDefinition` directly; React-local state only. New tree under `components/wizard-v2/` — `ReportBuilderWizardV2`, `BlockListV2`, six per-type block editors (`MarkdownBlockEditor`, `TableBlockEditor`, `MetricBlockEditor`, `ChartBlockEditor`, `CardBlockEditor`, `ActionsBlockEditor`), `FiltersEditorV2`, `DatasetsEditorV2`, `ViewsEditorV2`, `SourceEditor`, `VisibilityEditor`, `DatasetReconcileButton`. Pure `layoutOps.ts` keeps `blocks` + `layout` in sync via immutable updates. No `WizardState`, no serialization layer. ~1.6k LOC new.
- [ ] Delete `wizardSerialization.ts` and `wizardSerialization.test.ts` (50KB+). **Deferred to Phase 8** — old wizard still loads them.
- [ ] Delete `WizardBlock` / `WizardFilter` / `WizardState` from `wizardTypes.ts`. **Deferred to Phase 8** — old wizard still uses them.
- [x] Reuse the workflow `ConditionalStep` condition builder for `visibleWhen` / `hiddenWhen` / `disabledWhen` / `showWhen` editors. `VisibilityEditor` wraps the shared `ConditionEditor` and bridges legacy ↔ canonical via the existing `canonicalToLegacyCondition` / `legacyToCanonicalCondition` helpers in `reports/utils.ts`.
- [x] `reconcileDatasetBlock` becomes explicit "Reset block to dataset schema" button with diff preview — never silent on-load mutation. `DatasetReconcileButton` shows side-by-side JSON of current vs proposed and requires confirmation.
- [x] Move `connectionDefaults.ts` default-injection server-side. New `ReportService::normalize_definition_connections` runs on `get_report` / `create_report` / `update_report` / `validate_report` / `preview_report`. Picks the tenant's single active `postgres` connection when present; multi-connection tenants must still supply an explicit id (no behavior change for them). FE `withDefaultObjectModelConnection` stays as a redundant client-side belt for the legacy wizard but is now optional — `wizard-v2` doesn't depend on it. Full deletion in Phase 8.
- [x] Feature-flag new wizard; old wizard coexists for one release. `ReportPage` reads `searchParams.get('wizard')`; `?wizard=v2` mounts `ReportBuilderWizardV2`, otherwise legacy `ReportBuilderWizard`. Same lazy `Suspense` chunk strategy for both.

### Tests

- [x] Unit (TS): block list + layout ops — `wizard-v2/__tests__/layoutOps.test.ts` covers `collectLayoutBlockIds`, `orderedBlocksFromDefinition`, `addBlock`, `removeBlock`, `moveBlock`, `updateBlock`, deep-nested removal preserves structure, set-difference reorder. 11 tests.
- [x] Unit (TS): per-block-type editor — `wizard-v2/__tests__/blockEditors.test.tsx` covers `MarkdownBlockEditor` (emit on change + preserve unknown fields) and `TableBlockEditor` (render existing columns + preserve column metadata when adding). 4 tests. Per-step component coverage for the rest is deferred until those editors stop punting to legacy.
- [ ] Unit (TS): condition builder reuse — covered indirectly through `VisibilityEditor`; direct WASM-evaluator round-trip test deferred.
- [x] Integration (TS): load every corpus fixture into wizard, render without `onChange` firing. `wizard-v2/__tests__/losslessRoundTrip.test.tsx` loads all 11 fixtures from `tests/fixtures/reports/*.json`, mounts `ReportBuilderWizardV2`, and asserts no synthetic mutation on render. Persisted-shape lossless assertion (save-and-compare) deferred until v2 covers the deeper editors.
- [ ] Integration (TS): dataset reconcile flow — covered by the explicit `DatasetReconcileButton` UI but no automated test yet (renders a Dialog, awkward in jsdom). Deferred.
- [ ] E2E (Playwright): full author workflow. Deferred — runs against both wizards in Phase 8 once v2 reaches parity.

### Acceptance

- [x] Lossless-round-trip on 100% of corpus (render-side; save-side once deeper editors land).
- [ ] ~50KB of wizard code deleted. Deferred to Phase 8 — legacy wizard still ships.
- [ ] Opening + saving the same report is a no-op in git diff. Pending validate-after-save assertion in CI.
