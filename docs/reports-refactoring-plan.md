...
| 3 — Source provider trait | [x] Done. Trait + registry + three providers landed; render + validator dispatch routes through registry; four obsolete render leaves deleted. Aggregate-with-joins helper for object_model + the virtual aggregate engine still live in `reports.rs` (Phase 4 cleanup). |
...
**Status:** [x] Complete. Trait + registry + three providers landed in `services/reports/providers/`. `render_table_block`, `render_aggregate_table_block`, `render_aggregate_block`, and the block validator all dispatch through the registry. Four obsolete render leaves + their inline row-fetcher helpers (~600 LOC) deleted. The two pieces explicitly carried into Phase 4 (object-model's join-aware `aggregate_with_optional_joins` helper, and the virtual aggregate engine) stay in `reports.rs` for now — both depend on object-model DTO types and slot more naturally into Phase 4's single-render-pipeline work.

### Work

- [x] Define `ReportSourceProvider` trait with `fetch_rows`, `fetch_aggregate`, `validate_block`, `field_is_known`, `supports_aggregate_pushdown`, plus `table_columns` for response shaping.
- [x] Define `ProviderRegistry` and wire from service entry points — built once in `ReportService::new`, rebuilt in `with_runtime` so the late-bound `engine`/`runtime_client` reach the providers.
...
- [x] Create `services/reports/providers/workflow_runtime.rs` owning the Instances + Actions row-builder dispatch.
- [x] Create `services/reports/providers/system.rs` owning metric buckets, system snapshot, and the three rate-limit entities.
...
- [x] Move parallel `validate_workflow_runtime_block` / `validate_system_block` machinery into each provider's `validate_block`. The legacy free functions are now thin shims the provider impls call; the dispatcher in `validate_report_definition` goes through `self.providers.get(block.source.kind).validate_block(...)`.
- [x] Delete now-obsolete render leaves (`render_workflow_runtime_table_block`, `render_system_table_block`, `render_system_aggregate_table_block`, `render_system_aggregate_block`) + their five inline row-fetcher helpers (`system_rows_for_block`, `fetch_system_rows`, `runtime_execution_metric_rows`, `connection_rate_limit_status_rows`/`event_rows`/`timeline_rows`) and `apply_workflow_runtime_row_filters` — replaced by provider dispatch.
...
- [x] `services/reports.rs` shortened by ~600 LOC (four render leaves + their fetchers + `apply_workflow_runtime_row_filters` all gone). Still well above the aspirational 9,000-line target — the rest of the shrink lives in Phases 4-5 (single render pipeline + strict validator).
