...
| 6 — Edit-operation symmetry | [x] Done. `ReportEditOp` + `apply_edit_ops` in `runtara-report-dsl::edit_ops` with 10 variants applied atomically. New `POST /api/runtime/reports/{id}/edit` + `ReportService::edit_report`. New MCP `edit_report` tool. The 5 MCP layout-node tools now POST to `/edit`; 350 LOC of in-process walker helpers deleted. Legacy per-block REST handlers + MCP tools kept as parallel paths for one-release-lifespan migration. |
...
- [ ] REST per-op handlers become shims that build single-op batches — **deferred to Phase 8 cleanup**. The legacy per-block REST handlers (`POST /blocks`, `PUT/PATCH/DELETE /blocks/{id}`, `POST /blocks/{id}/move`) keep their existing direct-call code paths for one release lifespan, matching the plan's "one release lifespan" wording. Same for the MCP per-block tools that wrap them. Both will collapse to `/edit` calls in Phase 8 as part of the legacy cleanup.
...
- [ ] Integration (Rust): batched `/edit` equivalence to legacy per-op endpoints — **deferred to Phase 8 cleanup** (will become trivially true when the legacy handlers become single-op `/edit` shims).
