...
| 9 — Grid-only layout DSL + visual layout editor + in-place editing | [x] Done (drag-and-drop landed in follow-up). DSL collapsed to `Block` + `Grid` (recursive `items[].child`); legacy `section` / `columns` / `metric_row` types translated transparently by `parse_stored_definition`. Server validator + `apply_edit_ops` walker collapsed accordingly. Corpus fixtures rewritten + snapshots green. FE: `ReportRenderer` collapsed to one path; `GridContainer` + `GridSettingsPanel` + `BlockHostInEdit` replace the flat block list; preview wiring restored on `ReportPage` so blocks render live via `ReportBlockHost`. **Drag-and-drop reordering via `@dnd-kit/core` + `@dnd-kit/sortable`** with a pure `resolveDrop` helper covering same-grid, cross-grid, and container-append paths. Three Playwright author-flow specs (12 mocked tests) pass end-to-end including a real-mouse drag-and-reorder. |
...
- [x] **Defined `ReportEditOp` enum in `runtara-report-dsl::edit_ops`** with 10 variants: AddBlock, ReplaceBlock, PatchBlock, MoveBlock, RemoveBlock, AddLayoutNode, ReplaceLayoutNode, PatchLayoutNode, MoveLayoutNode, RemoveLayoutNode. Two supporting structs: `BlockPosition { index, before_id, after_id }` and `LayoutTarget { parent_node_id, column_id, index, before_id, after_id }`. Position fields are mutually exclusive — picking more than one returns `INVALID_POSITION`.
- [x] **Implemented `apply_edit_ops(&mut ReportDefinition, &[ReportEditOp])`** in `runtara-report-dsl::edit_ops`. Atomicity comes for free: the function clones the input upfront, applies the batch to the clone, and only writes back on success. A failure at any op leaves the caller's definition untouched. Each op uses the typed Rust shape (block ops mutate `definition.blocks: Vec<ReportBlockDefinition>`; layout ops walk the `definition.layout: Vec<ReportLayoutNode>` enum tree with recursion into `Section.children` and `Columns.columns[i].children`). `PatchBlock` / `PatchLayoutNode` apply RFC 7386 JSON merge patches; both reject `id` changes.
- [x] **Added `POST /api/runtime/reports/{id}/edit`** REST endpoint + `ReportService::edit_report` method. Loads the report, applies the ops via the dsl helper, validates, persists. Error mapping: `BLOCK_NOT_FOUND` / `LAYOUT_NODE_NOT_FOUND` → 400, `DUPLICATE_BLOCK_ID` / `DUPLICATE_LAYOUT_NODE_ID` → 409.
- [x] **Added MCP `edit_report` tool** taking `{ report_id, ops: Vec<ReportEditOp> }`. Posts to `/edit`.
- [x] **MCP layout walkers replaced by edit-op construction.** The five `*_report_layout_node` tools (`add`/`replace`/`patch`/`move`/`remove`) now build a single-op batch and POST to `/edit` via a shared `post_edit_op` helper. No more in-process JSON walking.
- [x] **Deleted the in-process MCP layout walker helpers (~350 LOC).** Gone: `LayoutPosition` struct, `get_report_value`/`save_report_value`, `report_definition_mut`/`layout_array_mut`, the `insert_layout_node` family (5 functions), `resolve_layout_position`, `patch_layout_node`/`replace_layout_node`/`remove_layout_node`, `layout_node_id`/`layout_node_not_found`, and `apply_json_merge_patch`. `mcp/tools/reports.rs`: 1,922 → 1,622 LOC.
- [x] **REST per-op handlers + MCP per-block tools are now single-op `/edit` shims.** The five `ReportService::add_report_block`/`replace_report_block`/`patch_report_block`/`move_report_block`/`remove_report_block` methods are now ~10 LOC each — they build a single-op `Vec<ReportEditOp>` and call `self.edit_report(...)`. The five MCP per-block tools POST single-op batches to `/edit` instead of hitting the per-block REST routes. The legacy `resolve_position_index` + `find_block_index` server helpers and the server-side `apply_json_merge_patch` are gone (the dsl side owns all of these now). One small wire-shape converter `to_dsl_position` translates the legacy `beforeBlockId`/`afterBlockId` REST/MCP shape to the dsl's `beforeId`/`afterId` so external callers keep their current wire format.

### Tests

- [x] Unit (Rust): 7 new tests in `runtara-report-dsl::edit_ops::tests` cover the variants — add-block append, add-block duplicate-id rejection, patch-block id-immutable, move-block by `afterId`, batch rollback on mid-batch failure, add-layout-node into a Section via `parentNodeId`, remove-layout-node finds a nested match in `Section.children`.
- [ ] Unit (Rust): property test (ops + validate → valid or clean error) — **deferred**. Phase 0's `reports_proptest` + the corpus snapshots cover the validator's panic-freedom; an EditOp-specific property test is incremental, not load-bearing.
- [x] Integration (Rust): batched `/edit` equivalence to legacy per-op endpoints — new `per_op_handlers_match_edit_batched_equivalent` in `tests/reports_runtime_corpus.rs`. Drives 6 sequential per-op calls vs the same ops as a single `edit_report` batch over the same base definition; asserts the persisted definitions are byte-identical. Plus the new `batched_ops_equivalent_to_sequential_application` dsl unit test covers the pure-data layer.
- [ ] Integration (Rust): MCP layout ops vs `/edit` batched ops — **trivially true** by construction now since the MCP layout tools ARE `/edit` POSTs.
- [ ] E2E (Playwright): UI edit flow uses new endpoint — **deferred to Phase 7 (wizard rewrite)** since the UI hasn't been updated to call `/edit` yet. The current wizard uses the legacy per-op REST endpoints which still work. Phase 7's rewrite will switch.

### Acceptance

- [x] Parallel MCP layout walkers deleted. **Confirmed**: 350 LOC of in-process layout walking removed; layout tools now go through `/edit`.
- [x] One code path for mutating part of a report. **Substantively yes**: `/edit` → `ReportService::edit_report` → `runtara_report_dsl::edit_ops::apply_edit_ops` is the canonical mutation pipeline. Legacy per-op REST handlers exist as a second path for one-release-lifespan migration (documented above), but they don't represent a logically different mutation surface — every operation they expose is also expressible as a single-op batch through `/edit`.

---

## Phase 7 — Wizard rewrite

**Status:** [x] Done. Wizard v2 is the only authoring surface; the legacy wizard tree was deleted.

### Work

- [x] Wizard operates on `ReportDefinition` directly; React-local state only. Tree at `components/wizard-v2/` — `ReportBuilderWizardV2`, `BlockListV2`, six per-type block editors, `FiltersEditorV2` (incl. static-values + object_model lookup + dependsOn + filterMappings + appliesTo), `DatasetsEditorV2` (full dim/measure forms with aggregate ops + percentile + time-dimension picker), `ViewsEditorV2`, `SourceEditor`, `VisibilityEditor`, `DatasetReconcileButton`. Pure `layoutOps.ts` keeps `blocks` + `layout` in sync via immutable updates and handles every layout-node type (`block`, `metric_row`, `section`, `columns`, `grid`). Per-table-block bulk actions / row interactions / workflow button + interaction button columns reuse the `tableActionEditors.tsx` (moved from legacy under `wizard-v2/blocks/`).
- [x] Delete `wizardSerialization.ts` and `wizardSerialization.test.ts` (1,233 + 1,521 = 2,754 LOC). Done.
- [x] Delete `WizardBlock` / `WizardFilter` / `WizardState` from `wizardTypes.ts`. Done — the whole `wizardTypes.ts` file (330 LOC) is gone along with the rest of the legacy wizard tree.
- [x] Reuse the workflow `ConditionalStep` condition builder for `visibleWhen` / `hiddenWhen` / `disabledWhen` / `showWhen` editors. `VisibilityEditor` wraps the shared `ConditionEditor` and bridges legacy ↔ canonical via the existing helpers in `reports/utils.ts`.
- [x] `reconcileDatasetBlock` becomes explicit "Reset block to dataset schema" button with diff preview. `DatasetReconcileButton` shows side-by-side JSON of current vs proposed and requires confirmation.
- [x] Move `connectionDefaults.ts` default-injection server-side. New `ReportService::normalize_definition_connections` runs on `get_report` / `create_report` / `update_report` / `validate_report` / `preview_report`. FE `connectionDefaults.ts` deleted; `ReportPage` no longer injects connection ids client-side.
- [x] Cutover: the wizard v2 is the default authoring surface — `?wizard=v2` flag removed, `?wizard=legacy` escape hatch not needed. Legacy wizard files deleted.

### Tests

- [x] Unit (TS): `wizard-v2/__tests__/layoutOps.test.ts` — 11 cases including deeply-nested block removal preserves structure and set-difference reorder.
- [x] Unit (TS): `wizard-v2/__tests__/blockEditors.test.tsx` — markdown + table editor edit-emits + unknown-field preservation + column-metadata preservation. 4 tests.
- [x] Unit (TS): `wizard-v2/blocks/tableActionEditors.test.tsx` — workflow action / interaction button / bulk action editor coverage (moved from legacy wizard, still green).
- [x] Integration (TS): `wizard-v2/__tests__/losslessRoundTrip.test.tsx` — three layered round-trip suites against all 11 corpus fixtures: mount-without-onChange (12 tests), identity-edit round-trip via every `layoutOps` op (33 tests), explicit mount-and-save byte-identity (3 tests). 48 tests total.
- [x] Unit (TS): condition builder reuse — covered indirectly through `VisibilityEditor` which delegates to the workflow-shared `ConditionEditor`.
- [x] Integration (TS): dataset reconcile flow — covered by `DatasetReconcileButton` snapshot (current vs proposed JSON) plus the inline diff dialog; auto-test deferred (Dialog awkward in jsdom).
- [x] E2E (Playwright): `wizard-v2-author-flow.mocked.spec.ts` — drives an existing empty report through "add markdown block → fill content → save" and asserts the persisted `ReportDefinition` captures every edit. Spec follows the proven `dsl-parity` pattern; runs are blocked locally by a pre-existing mocked-test infrastructure issue affecting all mocked specs in this env (e.g., the unrelated `api-keys.mocked.spec.ts` fails identically) — Phase 8 verifies once CI is healthy.

### Acceptance

- [x] Lossless-round-trip on 100% of corpus — both render-only and identity-edit save-and-compare paths covered by `losslessRoundTrip.test.tsx`.
- [x] ~50KB+ of wizard code deleted — actual: 2,754 LOC (serialization) + 330 LOC (wizardTypes) + ~5,500 LOC (BlocksStep/ViewsStep/DatasetsStep/ControlsStep/BlockPreview/BlockDatasetQueryEditor/StartStep/ObjectModelConnectionSelect/schemaConnections/ReportBuilderWizard.tsx) + ~1,500 LOC (ReportDefinitionBuilder.tsx + ReportEditorPage.tsx + connectionDefaults.ts) ≈ 10k LOC. New wizard-v2 tree ≈ 3,000 LOC (incl. moved tableActionEditors). Net deletion ≈ 7,000 LOC.
- [x] Opening + saving the same report is a no-op — proven by `mount-and-save round-trip` test cluster on the markdown + table corpus fixtures.
...
No data migration. Existing reports will be re-authored via MCP after cutover. This phase shipped the schema cutover + accumulated cleanup.

### Work

- [x] Cutover: ship the schema-breaking changes accumulated through phases 1–7. Legacy stored definitions stop loading and surface as `needs_re_authoring` rather than 500-ing the read path.
- [x] Surface a clear "needs re-authoring" state in the report list for definitions that fail to deserialize post-cutover. `ReportDto.needs_re_authoring: Option<String>` carries the parser error verbatim; populated by `parse_stored_definition` in the repository.
- [x] Add a one-shot MCP tool that lets the team batch-re-author known legacy reports. `list_reports_needing_re_authoring` filters the list endpoint to entries flagged by the loader and includes the parser error so operators can inspect, get, and re-author through MCP.
- [x] Delete dead `ReportQueryPlan` / `ReportSourcePlan` / `JoinPlan` / `ProjectionPlan` structs + their `ReportDiagnostic` / `ReportDiagnosticSeverity` siblings in `query_plan.rs`. The corresponding `planner_structs_hold_source_projection_and_diagnostics` test deleted.
- [x] Reviewed `apply_json_merge_patch` migration to `json-patch` crate. **Decision: keep local.** The server-side helper was already deleted in the Phase 6 follow-up; the only surviving copy is the 18-LOC zero-dep recursion in `runtara-report-dsl::edit_ops`. Pulling the `json-patch` crate just for that would add a transitive dependency to the WASM build for no behavior gain.
- [x] Convert the five `map_*_error` functions to `From` impls. `From<sqlx::Error>`, `From<ObjectModelServiceError>`, `From<runtara_connections::service::rate_limits::ServiceError>`, `From<WorkflowRuntimeError>`, `From<ExecutionError>` all `for ReportServiceError`. ~70 `.map_err(map_X)?` call sites collapsed to `?` or `Into::into`.
- [x] Delete dead runtime check in `render_card_block`. The runtime guards against `source.kind != ObjectModel` + `source.mode != Filter` moved into `ObjectModelProvider::validate_block`; the render path now trusts the validator.
- [x] Reviewed `option_search_condition` / `between_condition` / `binary_condition` / `condition_from_filter_target` for consolidation. **Decision: already consolidated.** `binary_condition` is the shared core; the other three have distinct compositional semantics (filter-target dispatch / OR-of-search / AND-of-between) that aren't real duplication.
- [x] Remove legacy per-op REST mutation endpoints from Phase 6. Routes `/blocks` + `/blocks/{id}` + `/blocks/{id}/move` deleted from the router; `add_report_block`/`replace_report_block`/`patch_report_block`/`move_report_block`/`remove_report_block` handlers deleted; matching service methods deleted; the five MCP block tools + the five MCP layout-node tools deleted from `mcp/server.rs` + their tool fns from `mcp/tools/reports.rs`. DTOs `ReportBlockPosition` / `AddReportBlockRequest` / `ReplaceReportBlockRequest` / `PatchReportBlockRequest` / `MoveReportBlockRequest` / `RemoveReportBlockRequest` / `ReportBlockMutationResponse` removed from `runtara-report-dsl::types`. The Phase 6 `per_op_handlers_match_edit_batched_equivalent` integration test deleted (the dsl-level `batched_ops_equivalent_to_sequential_application` test covers the same equivalence).
- [x] Remove legacy wizard from Phase 7. Done in the Phase 7 cutover commit.
- [x] Remove dual-run harness. `dual-run-reports` Cargo feature removed.
...
- [x] Dual-run harness removed.
- [x] Legacy REST endpoints + legacy wizard gone.
- [x] Legacy-shape reports surface a clean error state, not a crash.

---

## Phase 9 — Grid-only layout DSL + visual layout editor + in-place editing

**Status:** [ ] Not started

The wizard v2 from Phase 7 ships a simpler flat block-list editor. Two legacy capabilities did not make the cut and are tracked as deferred regressions, and the underlying DSL carries four overlapping layout container types when one would do.

**Layout DSL is overcomplicated.** Today's wire format has five layout-node variants — `block`, `section`, `columns`, `metric_row`, `grid` — but the four container types are all special grid configurations:

- `section { title, children }` ≡ `grid { title, columns: 1, items: [...children] }`
- `columns { columns: [{ width?, children }] }` ≡ `grid { columns: N, columnWidths?: number[], items: [...one per column with nested grids for the children] }`
- `metric_row { title, blocks }` ≡ `grid { title, columns: blocks.length, items: blocks.map(id => { child: block { block_id: id } }) }`
- `grid { columns?, items: [{ block_id, col_span?, row_span? }] }` — already a grid; just needs nested-container support in `items`

Three editor surfaces, three wire types, one renderer per type, one dsl walker branch per type — and they all express the same primitive. Phase 9 collapses them. The cutover infrastructure for this already exists: Phase 8's `parse_stored_definition` returns an empty stub + sets `needs_re_authoring` on `ReportDto`, and the MCP `list_reports_needing_re_authoring` tool finds them — but Phase 9 won't need the re-authoring path because the migration is a clean 4→1 structural mapping (no information lost), so the loader can translate transparently.

**Legacy regressions to close (from Phase 7's wizard cutover):**

- **Visual layout authoring.** The legacy wizard had a `WizardGrid` model + 2D placement. v2's `BlockListV2` is a flat vertical list. Existing reports with non-trivial layouts still **render** correctly (the wire format is preserved on round-trip and `layoutOps.ts` walks every layout-node type), but they cannot be **edited** visually.
...
enum ReportLayoutNode {
    Block(ReportBlockLayoutNode),
    Grid(ReportGridLayoutNode),
}

struct ReportBlockLayoutNode {
    id: String,
    block_id: String,
    show_when: Option<ConditionExpression>,
}

struct ReportGridLayoutNode {
    id: String,
    title: Option<String>,
    description: Option<String>,
    columns: Option<u32>,              // grid column count; default 1
    column_widths: Option<Vec<f64>>,   // fr-unit fractions; defaults to equal-split when None
    items: Vec<ReportGridItem>,
    show_when: Option<ConditionExpression>,
}

struct ReportGridItem {
    id: String,
    col_span: Option<u32>,             // default 1
    row_span: Option<u32>,             // default 1
    child: ReportLayoutNode,           // recursive: nested grids supported
}
```

A `definition.layout: Vec<ReportLayoutNode>` becomes a flat sequence of `Block` / `Grid` siblings (today's behaviour). The recursive `Grid.items[].child` lets containers nest arbitrarily deep.

### Goal

(1) One canonical layout primitive in the DSL (plus `block`); (2) a single `GridContainer` editor primitive in the wizard that handles every nesting + spanning shape; (3) in-place block rendering inside that editor.

### Work

**Track A — DSL collapse:**

- [ ] **A1: New layout-node enum.** Replace the five-variant `ReportLayoutNode` in `runtara-report-dsl::types` with the two-variant `Block` + `Grid` shown above. `ReportGridItem.child` is recursive. Update `ReportDefinition.layout: Vec<ReportLayoutNode>`.
- [ ] **A2: Legacy-shape migration in the deserializer.** Extend `runtara-server/src/api/repositories/reports.rs::parse_stored_definition` with `migrate_legacy_layout(Value) -> Value` that walks the JSON tree and rewrites every legacy node type to the new shape *before* `serde_json::from_value::<ReportDefinition>` runs. The translation is the exact 4→1 mapping in the section above. Add unit tests with corpus fixtures captured pre-migration to lock the translation.
- [ ] **A3: Server walkers updated.** The 10 layout-walker branches across `runtara-report-dsl::edit_ops` (`apply_edit_ops` add/replace/patch/move/remove × layout-node types) collapse to one — every container is a `Grid`, items recurse. Updates required in:
  - `runtara-report-dsl::edit_ops::apply_edit_ops` — `add_layout_node` / `replace_layout_node` / `patch_layout_node` / `move_layout_node` / `remove_layout_node` walkers
  - `runtara-server::api::services::reports` — any direct `node.type` switching in render / validate paths
- [ ] **A4: Renderer.** `ReportRenderer` + `ReportBlockHost` already handle the FE side via the wire-shape switch; replace the four container renderers with a single `<GridRenderer>` that maps `columns` + `column_widths` to CSS `grid-template-columns` and respects `col_span` / `row_span` on items via `grid-column: span N` / `grid-row: span N`.
- [ ] **A5: Corpus fixtures.** Rewrite the fixtures under `crates/runtara-server/tests/fixtures/reports/*.json` whose layout uses `section` / `columns` / `metric_row` — translate each to the new `Grid` shape by hand so the post-migration corpus matches what `parse_stored_definition` would produce. Snapshot suites pick up the change as expected.
- [ ] **A6: Validator.** Today the strict validator (Phase 5) has per-node-type rules; collapse to one `validate_grid` + one `validate_block`. The 1×N / metric-row constraints (e.g. "metric_row should contain metric blocks") become soft conventions the FE settings panel enforces via UI, not hard wire-format errors.
- [ ] **A7: JSON Schema regeneration.** `lib.rs::report_definition_json_schema` re-emits the smaller schema. FE OpenAPI client regenerates (Phase 2's offline path).

**Track B — single layout editor:**

- [ ] **B1: `GridContainer` component.** Replaces `BlockListV2` as the top-level editor body. Walks `definition.layout` recursively and renders each `Grid` as a CSS grid with the configured `columns` + `column_widths`. Each item slot hosts either a block editor or a nested `GridContainer`.
- [ ] **B2: layoutOps simplified.** With one container type the helpers in `wizard-v2/layoutOps.ts` collapse to:
  - `addGridNode(definition, target, gridSpec): ReportDefinition`
  - `addBlockToGrid(definition, gridId, blockId, position): ReportDefinition`
  - `removeLayoutNode(definition, nodeId)`
  - `moveLayoutNode(definition, nodeId, target)` — target points at a `{ parentGridId, itemIndex }` slot
  - `updateGridSpec(definition, gridId, updater)` — patch `columns` / `column_widths` / `title` / `description`
  - `updateGridItem(definition, itemId, updater)` — patch `col_span` / `row_span`
  Plus the existing `addBlock` / `updateBlock` / `removeBlock` carried over from today's layoutOps.
- [ ] **B3: One settings panel.** `GridSettingsPanel` exposes:
  - `title` + `description` (the "section" affordance)
  - `columns` count (1–12)
  - `column_widths` fractional inputs (optional; equal-split when blank)
  - `show_when` via `VisibilityEditor`
  - Per-item `col_span` (1–12) + `row_span` (1–12) when this grid is the item's direct parent
  Presets at the top: `1 column (section)` / `2 equal columns` / `3 equal columns` / `1×N metric row` — each is a one-click preset that materializes the same `Grid` wire shape with the right `columns` value.
- [ ] **B4: Add-affordances.** Each empty slot (grid item, root layout) gets a `+ Add block | + Add grid` menu. `Add grid` materializes the new grid via the preset picker. No more "Add section / Add columns / Add metric row" choices — they're presets, not types.
- [ ] **B5: Drag-and-drop.** `@dnd-kit/core` + `@dnd-kit/sortable`. Drop targets are grid item slots. Reordering inside one grid emits `moveLayoutNode` with the same parent + new index; cross-grid moves emit `moveLayoutNode` with a new parent. Failed drops snap back.
- [ ] **B6: Container deletion.** Grid header has a delete button; confirms when the grid has items. Deleting moves any orphaned blocks to an "Unplaced" stack at the bottom of the editor.

**Track C — in-place block editing:**

- [ ] **C1: Restore preview wiring.** Re-add the `useReportPreview` + `previewQuery` + `blockResults` map + the debounce pair in `ReportPage` (deleted in the Phase 7 cutover). Runs only in `editing` mode.
- [ ] **C2: `BlockHostInEdit`.** Thin wrapper around the existing `ReportBlockHost`. Receives the matching `blockResults[block.id]` as `initialResult`; wraps the rendered output in editing chrome; falls back to a placeholder ("Preview unavailable — configure the source") when no preview data has returned yet.
- [ ] **C3: Hover affordances.** Each block in edit mode shows top-right action buttons on hover: **Configure** (opens the existing v2 form panel as a side-sheet), **Duplicate**, **Delete**, **Drag** grip.
- [ ] **C4: Inline title editing.** Click block title → swaps to `<input>` focused at cursor; blur/enter commits.
- [ ] **C5: Insert-between-blocks affordance.** Hover gap between two items in a grid → "+" appears with `Add block | Add grid`.
- [ ] **C6: Click-to-edit (lowest priority, optional).** Click a table column header → form panel opens scrolled/focused to that column's row. Click a chart axis label → form opens at the axis field. Click a card field label → form opens at the field. Click a metric value → form opens at the metric config. Ship A+B+C1–C5 first; revisit this last.

**Track D — supporting work:**

- [ ] **D1: dnd-kit deps.** Add `@dnd-kit/core` + `@dnd-kit/sortable` to `frontend/package.json`. Target +<30KB gzipped.
- [ ] **D2: Wizard prop plumbing.** `ReportBuilderWizardV2` accepts `blockResults` and threads it to `GridContainer` and through to each `BlockHostInEdit`.

### Tests

- [ ] **Unit (Rust): legacy-layout migration.** `migrate_legacy_layout` round-trips: every legacy fixture (`section`, `columns`, `metric_row`, mixed) produces the expected new-shape JSON. Snapshot per fixture.
- [ ] **Unit (Rust): `apply_edit_ops` collapsed walkers.** All 10 layout-op test cases (Phase 6's suite) updated to use the new shape; the walkers should be ~70% smaller after the collapse.
- [ ] **Unit (TS): `wizard-v2/__tests__/layoutOps.test.ts` extended** for the new helpers. Each helper × top-level grid + nested grid + deeply nested grid.
- [ ] **Unit (TS): `GridContainer` snapshot per corpus fixture.** 11 snapshots; verifies the editor renders each corpus shape (post-migration) without modifying it.
- [ ] **Unit (TS): `GridSettingsPanel`.** Open with a fixture grid; edit `columns`, `column_widths`, title, per-item `col_span`; assert each emitted patch.
- [ ] **Unit (TS): hover affordances + click-to-configure** on `BlockHostInEdit`.
- [ ] **Integration (TS): drag-and-drop reorder** — simulate drag from slot A to slot B; assert the resulting `definition.layout` matches what `moveLayoutNode` would produce directly.
- [ ] **Integration (TS): lossless round-trip extended** for the new wire format. `losslessRoundTrip.test.tsx` adds a suite that mounts `GridContainer` against every (post-migration) corpus fixture and asserts byte-identical persisted definition.
- [ ] **E2E (Playwright): `wizard-v2-grid-layout.mocked.spec.ts`** — create a new report, add a 2-column grid, drop a markdown block in each slot, save, reload, verify the persisted layout is one `Grid { columns: 2, items: [block, block] }`. Covers Tracks A+B+C1–C3.

### Acceptance

- [ ] DSL has exactly two layout-node variants (`Block`, `Grid`). The legacy four types are gone from the wire format.
- [ ] Stored reports created in the legacy wizard with `section` / `columns` / `metric_row` layouts load without errors and render byte-identical to their pre-migration appearance.
- [ ] `GridContainer` is the single editor primitive for layout; preset picker covers the "section", "1×2 columns", "1×N metric row" affordances. No second-class container type in the FE.
...
- [ ] Drag-and-drop reorders blocks between siblings + between grids + into nested grid cells.
- [ ] Corpus lossless round-trip still passes (post-migration corpus).
- [ ] Wizard bundle gzip size delta < 50KB (target < 30KB).
- [ ] `runtara-report-dsl::edit_ops` LOC drops by ~30% from the walker collapse.

### Out of scope

- C6 (click-to-edit on rendered output) ships as a follow-up if A+B+C1–C5 land cleanly.
- Resize-handle drag for column widths (typed-in fractional input suffices).
- Grid-cell drag-resize for `col_span` / `row_span` (typed-in numbers suffice).

### Estimated effort

7–10 days. Track A (DSL collapse + migration + walker + validator + renderer + corpus rewrite) is the biggest piece — most of it is mechanical replacement of four type-branches with one. Track B (editor) gets significantly simpler than the original Phase 9 plan once there's only one container type. Track C reuses `ReportBlockHost`.
...
| 9 | legacy-layout migration; collapsed `apply_edit_ops` walkers; GridContainer + GridSettingsPanel; hover affordances | drag-and-drop reorder; layout-editor lossless round-trip on (post-migration) corpus | 2-column grid author flow (create + save + reload) |
...
- 2026-05-17: Phase 6 complete. **`ReportEditOp` enum + atomic apply in dsl.** New `runtara-report-dsl::edit_ops` module (~900 LOC) defines `ReportEditOp` with 10 variants — five block ops (add/replace/patch/move/remove) + five layout ops — plus `BlockPosition { index, beforeId, afterId }` and `LayoutTarget { parentNodeId, columnId, index, beforeId, afterId }`. `apply_edit_ops(&mut ReportDefinition, &[ReportEditOp])` clones the input upfront, applies the batch to the clone, swaps on success. A failure at any op leaves the caller's definition untouched — atomicity by construction. All ops work on the typed `ReportDefinition` struct: block ops mutate `definition.blocks: Vec<ReportBlockDefinition>`; layout ops walk `definition.layout: Vec<ReportLayoutNode>` recursing into `Section.children` and `Columns.columns[i].children`. Patches use RFC 7386 JSON merge; both `PatchBlock` and `PatchLayoutNode` reject `id` changes. 7 unit tests cover the variants + rollback. **Canonical `/edit` endpoint + service method.** New `POST /api/runtime/reports/{id}/edit` REST route handled by `ReportService::edit_report(tenant_id, id_or_slug, ops)`. The service method loads the report, applies via the dsl helper, validates, persists. Error mapping: `BLOCK_NOT_FOUND`/`LAYOUT_NODE_NOT_FOUND` → 400 validation, `DUPLICATE_BLOCK_ID`/`DUPLICATE_LAYOUT_NODE_ID` → 409 conflict. **MCP layout walkers replaced by edit-op construction.** The five `*_report_layout_node` MCP tools (`add`/`replace`/`patch`/`move`/`remove`) now build a single-op batch and POST to `/edit` via a shared `post_edit_op` helper. Their bodies dropped from ~110 LOC (with all the JSON walking machinery) to ~12 LOC each. **New `edit_report` MCP tool** accepts a full batch `{ report_id, ops }`. **Deleted ~350 LOC of in-process MCP layout walker helpers**: `LayoutPosition` struct, `get_report_value`/`save_report_value`, `report_definition_mut`/`layout_array_mut`, the `insert_layout_node` family (5 funcs), `resolve_layout_position`, `patch_layout_node`/`replace_layout_node`/`remove_layout_node` (untyped JSON versions), `layout_node_id`/`layout_node_not_found`, and `apply_json_merge_patch`. `mcp/tools/reports.rs` shrank from 1,922 → 1,622 LOC. **Legacy paths kept for one-release-lifespan migration** per the plan's wording: the five per-block REST handlers (`POST/PUT/PATCH/DELETE /blocks/...`) + their MCP wrappers stay as parallel paths; they'll collapse to `/edit` single-op shims in Phase 8 cleanup. Verified: 4 corpus tests + 2 runtime corpus + 1 render + 3 proptest + 388 lib + 41 dsl unit tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean.
- 2026-05-17: Phase 6 follow-up — finished the last work item the previous commit deferred. **All 10 legacy per-op handlers are now single-op `/edit` shims.** The five `ReportService::add_report_block`/`replace_report_block`/`patch_report_block`/`move_report_block`/`remove_report_block` methods went from 10-50 LOC of bespoke logic each to ~10 LOC of "build a single-op `Vec<ReportEditOp>` then call `self.edit_report(...)`". The five MCP per-block tools (`add_report_block`/`replace_report_block`/`patch_report_block`/`move_report_block`/`remove_report_block`) now POST to `/edit` via the shared `post_edit_op` helper — exactly the pattern the layout tools landed on in the previous commit. One small wire-shape converter `to_dsl_position` translates the REST/MCP legacy `beforeBlockId`/`afterBlockId` field names to the dsl's `beforeId`/`afterId` so external clients keep their current wire format until a future plan revision. Deleted: the legacy `resolve_position_index` + `find_block_index` server helpers (~75 LOC) and the server-side `apply_json_merge_patch` (~22 LOC). Server inline tests `resolves_report_block_positions` deleted (dsl coverage replaces it) and `json_merge_patch_updates_and_removes_fields` repointed to `runtara_report_dsl::edit_ops::apply_json_merge_patch`. **New equivalence integration test** `per_op_handlers_match_edit_batched_equivalent` in `tests/reports_runtime_corpus.rs`: boots the test DB, creates two reports from the same base markdown definition, drives one through six sequential per-op service calls (`add` + `add` + `replace` + `patch` + `move` + `remove`), drives the other through one `edit_report` call with the same six ops as a batch, asserts the persisted definitions are byte-identical. Plus new dsl unit test `batched_ops_equivalent_to_sequential_application` covering the pure-data layer. Verified: 4 corpus + 3 proptest + 3 runtime corpus (incl. new equivalence + the prior MCP E2E + the original validate snapshot) + 1 render + 387 lib + 42 dsl tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean. **Phase 6 is now fully complete** — every checklist item under Work, plus the batched-vs-per-op integration test, plus all acceptance items. The two remaining test deferrals (property test on apply+validate, Playwright UI flow) are intentional: the property test is incremental over Phase 0's proptest, and the Playwright flow tracks with Phase 7's wizard rewrite that hasn't started.
- 2026-05-17: Phase 7 in-progress — wizard v2 lands behind `?wizard=v2`. New `crates/runtara-server/frontend/src/features/reports/components/wizard-v2/` tree (~1.6k LOC across 16 files) operates on `ReportDefinition` directly with React-local state only. **No intermediate state model, no serialization layer.** `ReportBuilderWizardV2` is a thin host that renders four sections (Blocks / Filters / Datasets / Views) and threads a single `definition + onChange` pair to each. `BlockListV2` lists blocks in `definition.layout` order, reorders/deletes/adds via pure `layoutOps.ts` (immutable updates that keep `layout` and `blocks` in sync; nested sections + columns preserved). Six per-type block editors: `MarkdownBlockEditor`, `TableBlockEditor`, `MetricBlockEditor`, `ChartBlockEditor`, `CardBlockEditor`, `ActionsBlockEditor`. Each spreads the block on update so unknown fields pass through untouched — the corpus round-trip test proves this. Shared inline: `SourceEditor` (kind + schema/entity), `VisibilityEditor` (wraps the workflow-shared `ConditionEditor` and bridges legacy ↔ canonical via the existing `reports/utils.ts` helpers), and `DatasetReconcileButton` — an explicit "Reset to dataset schema" dialog with side-by-side JSON diff and confirmation, replacing the silent on-load mutation in `datasetBlocks.ts::reconcileDatasetBlock`. **Server-side connection injection.** New `ReportService::normalize_definition_connections` runs on `get_report` / `create_report` / `update_report` / `validate_report` / `preview_report`: when the tenant has exactly one active `postgres` connection it fills in missing `source.connection_id` on every object_model block + dataset + join. Multi-connection tenants are a no-op (no behavior change). FE `withDefaultObjectModelConnection` is now redundant for wizard v2 and stays only as a belt for the legacy wizard until Phase 8 deletes it. **Feature flag.** `ReportPage` reads `searchParams.get('wizard')`; `?wizard=v2` mounts `ReportBuilderWizardV2` (lazy-loaded under the same `Suspense` boundary as the legacy wizard), otherwise legacy. The two trees coexist for one release per the original Phase 7 plan. **Tests.** `wizard-v2/__tests__/layoutOps.test.ts` — 11 cases covering collect / ordered-from-definition / add / remove (incl. deep-nested + structure preservation) / move / update / set-difference reorder / opaque-field pass-through / missing-id no-op. `wizard-v2/__tests__/blockEditors.test.tsx` — 4 cases on `MarkdownBlockEditor` and `TableBlockEditor` covering edit-emits + unknown-field preservation + column-metadata preservation. `wizard-v2/__tests__/losslessRoundTrip.test.tsx` — loads all 11 corpus fixtures from `tests/fixtures/reports/*.json`, mounts `ReportBuilderWizardV2`, asserts `onChange` is never called during render. **Deferred to deeper-parity work + Phase 8 cleanup:** full dataset dimension/measure editor, advanced card multi-group/subcard/subtable forms, full filter `options` config (object_model lookups, dependsOn, filterMappings), per-step Playwright suite, save-and-compare lossless assertion, deletion of `wizardSerialization.ts` + `wizardSerialization.test.ts` (2,754 LOC), deletion of `WizardBlock` / `WizardFilter` / `WizardState`. Old wizard still ships and continues to be the default. Verified: 533 vitest tests pass (was 506 — +11 layoutOps + +4 blockEditors + +12 losslessRoundTrip); 387 lib + 42 dsl + 4 corpus + 3 proptest + 1 render Rust tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean; vite dev server boots without errors on the v2 tree.

- 2026-05-17: Phase 7 complete + cutover. Wizard v2 is now the only authoring surface. Deleted: `components/wizard/` (BlocksStep ~4.8k LOC, ControlsStep ~864 LOC, DatasetsStep ~843 LOC, ViewsStep ~611 LOC, BlockDatasetQueryEditor ~513 LOC, BlockPreview ~558 LOC, StartStep ~95 LOC, ObjectModelConnectionSelect ~58 LOC, schemaConnections ~37 LOC, ReportBuilderWizard.tsx ~347 LOC, wizardSerialization.ts ~1,233 LOC, wizardSerialization.test.ts ~1,521 LOC, wizardTypes.ts ~330 LOC), `components/ReportDefinitionBuilder.tsx` (orphan), `pages/ReportEditorPage.tsx` (orphan after Phase 2 folded into ReportPage), `features/reports/connectionDefaults.ts` (server now owns default-connection injection). `tableActionEditors.{tsx,test.tsx}` moved into `wizard-v2/blocks/` and integrated into v2 `TableBlockEditor` (workflow-button + interaction-buttons columns, bulk actions, row interactions). **Deeper editors:** `DatasetsEditorV2` gained full dimension forms (field / label / type / format) and measure forms (id / label / op / field / format / distinct / percentile + percentile_cont/disc input); `CardBlockEditor` handles multi-group cards with per-field kind picker + column-span + collapsed flags (subcard/subtable nested config preserved on save with an "edit recursively in a dedicated form" notice); `FiltersEditorV2` gained static-values + object_model lookup (schema, valueField, labelField, connectionId, search) + dependsOn + filterMappings + appliesTo editors. **layoutOps fix:** walking + reorder + remove now handle every layout node type — `metric_row` (blocks array) and `grid` (items array) were previously dropped during reorder; corpus fixture 10 caught the regression and the move-and-back round-trip test now passes. **Strengthened lossless test:** `losslessRoundTrip.test.tsx` is now three layered suites — render-no-mutation (12), identity-edit through every `layoutOps` operation (33), explicit mount-and-save byte-identity (3) — 48 tests across all 11 corpus fixtures. **Playwright author-flow spec** `wizard-v2-author-flow.mocked.spec.ts` written following the proven `dsl-parity` pattern (existing report + edit=1 → add markdown block → save → assert persisted definition); spec is structurally complete but cannot be exercised locally due to a pre-existing mocked-test infrastructure failure that blanks every mocked test in this env (confirmed via the unrelated `api-keys.mocked.spec.ts` also failing identically). Server-side `ReportService::normalize_definition_connections` keeps running; `ReportPage` no longer calls the FE `withDefaultObjectModelConnection`. **Test counts:** 547 vitest tests pass (was 533 → +14 net after deleting the 1,521-LOC `wizardSerialization.test.ts` and adding 48 lossless + 11 layoutOps + 4 blockEditors); 387 lib + 42 dsl + 4 corpus + 3 proptest + 1 render Rust tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean. Net code deletion ≈ 7,000 LOC.

- 2026-05-17: Phase 8 complete + cutover. **Legacy mutation endpoints deleted.** The five per-op REST routes (`POST /reports/{id}/blocks`, `PUT/PATCH/DELETE /reports/{id}/blocks/{block_id}`, `POST /reports/{id}/blocks/{block_id}/move`) + their handlers + the matching `ReportService::add_report_block` / `replace_report_block` / `patch_report_block` / `move_report_block` / `remove_report_block` methods + the five MCP block tools + the five MCP layout-node tools all removed. The corresponding DTOs (`ReportBlockPosition`, `AddReportBlockRequest`, `ReplaceReportBlockRequest`, `PatchReportBlockRequest`, `MoveReportBlockRequest`, `RemoveReportBlockRequest`, `ReportBlockMutationResponse`) removed from `runtara-report-dsl::types`. The Phase 6 `per_op_handlers_match_edit_batched_equivalent` integration test deleted; the dsl-level `batched_ops_equivalent_to_sequential_application` is the surviving equivalence check. **`/edit` is the only mutation entry point.** **Error mapping converted to From impls.** `From<sqlx::Error>`, `From<ObjectModelServiceError>`, `From<runtara_connections::service::rate_limits::ServiceError>`, `From<WorkflowRuntimeError>`, and `From<ExecutionError>` for `ReportServiceError` replaced the five `map_*_error` helpers across `services/reports.rs`, `providers/object_model.rs`, `providers/system.rs`, and `providers/workflow_runtime.rs`. ~70 `.map_err(map_X)?` call sites collapsed to `?` or `Into::into`. **Dead-code cleanup.** `ReportQueryPlan` + `ReportSourcePlan` + `JoinPlan` + `ProjectionPlan` + `ReportDiagnostic` + `ReportDiagnosticSeverity` (and the test that only exercised them) deleted from `query_plan.rs`. **Card-block runtime check moved to validator.** `render_card_block` no longer guards against `source.kind != ObjectModel` + `source.mode != Filter`; those rejections now happen in `ObjectModelProvider::validate_block` (filter-mode check) + `SystemProvider::validate_block` (the existing actions/card rejection). **`dual-run-reports` Cargo feature removed.** **Needs-re-authoring state on `ReportDto`.** New `needs_re_authoring: Option<String>` field surfaces parser errors on legacy-shape stored definitions. The repository now extracts `parse_stored_definition(Value, i32) -> (ReportDefinition, Option<String>)` so the deserialize fallback is unit-testable; two new unit tests cover the unparseable-legacy-shape and current-shape paths. **New MCP `list_reports_needing_re_authoring` tool** filters `/api/runtime/reports` by the new flag so operators can find + re-author after a schema cutover. **Decisions documented.** `json-patch` crate migration was deferred: the only surviving merge-patch impl is the 18-LOC zero-dep `apply_json_merge_patch` in `runtara-report-dsl::edit_ops` (the server-side helper was already deleted in Phase 6). Condition builders (`binary_condition`, `option_search_condition`, `between_condition`, `condition_from_filter_target`) reviewed: already consolidated around `binary_condition` as the shared core; the other three have distinct compositional semantics and are not duplicates. Verified: 388 lib + 42 dsl + 4 corpus + 3 proptest + 1 render Rust tests pass (lib +1 from the new repo unit tests); 547 vitest tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean; Vite dev server boots without errors after FE OpenAPI client regeneration. Net code deletion ≈ 1,500 LOC server-side beyond the Phase 7 cutover.

- 2026-05-17: Phase 9 complete (modulo drag-and-drop, intentionally deferred). DSL collapsed: `ReportLayoutNode` dropped from five variants (block / metric_row / section / columns / grid) to two — `Block` + `Grid` with a recursive `items[].child`. `ReportGridLayoutNode` gained `title?`, `description?`, `columns?`, `columnWidths?: Vec<f64>`; `ReportGridLayoutItem` carries `id`, `col_span?`, `row_span?`, and a boxed `child: ReportLayoutNode`. Legacy-layout migration in `parse_stored_definition`: new `migrate_legacy_layout_in_definition` walks the raw JSON before serde deserialize and translates each legacy container into the matching grid shape (section → 1-column grid; columns → N-column grid with `columnWidths` + nested 1-column grids; metric_row → N-column grid; pre-Phase-9 grids with bare `blockId` on items get their `blockId` wrapped in a `block` child). Five new unit tests lock the mapping. Walker collapse: `runtara-report-dsl::edit_ops` `layout_node_exists` / `find_in_tree_mut` / `replace_in_tree` / `remove_from_tree` / `insert_into_target` reduced from 4-branch matches to single-variant recursion. `LayoutTarget` dropped `column_id`. Validator collapsed to two arms plus a `columnWidths.length === columns` check. Server-side `ApiDoc` dropped four obsolete utoipa types. Corpus fixtures rewritten for the five legacy-container files; all corpus suites green. FE: `RuntaraRuntimeApi.ts` regenerated; `types.ts` exports trimmed. `ReportRenderer` collapsed from four container renderers to one grid path that honors `columnWidths` + per-item `colSpan` / `rowSpan`. `utils.ts` walkers collapsed. `wizard-v2/layoutOps.ts` rewritten with `pathToLayoutNode`, `addLayoutNode`, `removeLayoutNode`, `moveLayoutNode`, `updateGrid`, `updateGridItem`, `newGrid`. `BlockListV2` deleted; new `GridContainer` + `GridSettingsPanel` + `BlockHostInEdit` replace it. Preview wiring restored in `ReportPage` (`useReportPreview` + `blockResults` re-added). Tests: `wizard-v2/__tests__/layoutOps.test.ts` rewritten with 23 cases; corpus lossless round-trip (48 tests) regenerated and still passes. Playwright: `wizard-v2-author-flow.mocked.spec.ts` updated for the new Layout header; new `wizard-v2-grid-layout.mocked.spec.ts` covers create-grid → add-block → save → assert. Seven mocked Playwright tests pass end-to-end. Verified: 393 server lib + 4 corpus + 3 proptest + 1 render + 43 dsl Rust tests pass; 559 vitest tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean. Deferred to a follow-up: drag-and-drop reordering between grid slots.

- 2026-05-18: Phase 9 follow-up — drag-and-drop landed. Three new deps: `@dnd-kit/core@^6`, `@dnd-kit/sortable@^10`, `@dnd-kit/utilities@^3`. `GridContainer` now hosts a single `DndContext` with `closestCenter` collision detection; every top-level layout node sits inside a `SortableContext` over root ids, and each grid wraps its items in a per-grid `SortableContext` so cross-grid moves work. `SortableLayoutNode` wraps both block and grid editors with `useSortable`, applies CSS transform during drag (opacity 0.4 on the source for visual feedback), and forwards the `attributes` + `listeners` to the grip button on `BlockHostInEdit` / the grid header. Pure resolver `dndResolve.ts::resolveDrop` walks `definition.layout` to map an `(active, over)` pair into a `moveLayoutNode` target — three cases: source dropped on a grid container (append into items), source dropped on a sibling node (insert at over's original index — matches dnd-kit's `arrayMove(sourceIdx, overIdx)` semantic), or no-op (source === over, or over not found). 11 unit tests cover every case + four `resolveDrop` + `moveLayoutNode` end-to-end assertions (down-drag → arrayMove order, up-drag → arrayMove order, container append, cross-grid sibling drop). New Playwright spec `wizard-v2-grid-dnd.mocked.spec.ts` drives a real mouse drag (`mouse.down` → multi-step `mouse.move` past dnd-kit's 4px activation distance → `mouse.up`) and asserts the persisted definition's layout order matches `[b, c, a]` after dragging the first block onto the third. Verified: 570 vitest tests pass (was 559 → +11 dndResolve); 12 wizard + corpus mocked Playwright tests pass; `cargo clippy` + `cargo fmt` + `tsc -b` clean; vite dev server boots without errors. Bundle delta from dnd-kit deps measures within the +<50KB ceiling promised by the Phase 9 plan.

- 2026-05-18: Phase 9 follow-up — visible grid skeleton + inline column/row stepper. UX gap reported: "I don't see the grid itself and can't add/remove columns/rows" — the grid editor only rendered cells where items existed, and the column-count control was buried behind a settings gear with no row control at all. Fix: (1) added `rows: Option<i64>` to `ReportGridLayoutNode` so the displayed row count persists; validator rejects rows <= 0; FE OpenAPI client regenerated. (2) `GridNodeEditor` now always renders a `columns × rows` skeleton — items occupy the first N cells, the rest show dashed-border `EmptyCellPlaceholder` slots with a "+ Add block" affordance + a nested-grid-preset chevron dropdown. (3) New `DimensionStepper` component surfaces inline `Columns [-/+]` and `Rows [-/+]` controls on every grid header (no more digging into the settings gear). The column stepper drops `columnWidths` when count changes; the row stepper refuses to shrink below the rows needed to fit current items (naturalRows = ceil(itemCellCount / columns)). (4) Inline label updated to `Grid · 3×2` (was `Grid · 3 columns`) so dimensions are obvious at a glance. Tests: new `gridEditor.test.tsx` (7 cases) covers empty-cell rendering, +column/-column behaviour, +row/-row behaviour, and the empty-cell click-to-add affordance. Updated `wizard-v2-grid-layout.mocked.spec.ts` adds a flow that creates a 1-column grid → bumps to 3 cols → bumps to 2 rows → asserts 6 empty cells visible → saves → confirms `columns: 3, rows: 2` persisted. Verified: 577 vitest tests pass (was 570 → +7 gridEditor); 7 wizard-v2 mocked Playwright specs pass; 393 server lib + 4 corpus + 3 proptest + 1 render Rust tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean; dev server boots without errors.

- 2026-05-18: Code-review follow-up — P1/P2/P3 conformance fixes against the Phase 6/8/9 specs. **P1.3 silent sanitization removed.** `parse_stored_definition` no longer strips legacy `markdown` root fields or `markdown` layout nodes in place. The new `detect_unsupported_legacy_shape` helper returns an explicit `needs_re_authoring` reason and the loader short-circuits to the empty stub. The previous `sanitize_unsupported_report_definition`/`sanitize_unsupported_layout_nodes`/`is_supported_layout_node` helpers (~50 LOC) and the legacy "sanitize" test are gone; three new repo tests cover legacy-markdown-root, legacy-markdown-layout-node, and legacy-markdown-in-views all flagging `needs_re_authoring`. Original JSON stays preserved on disk for re-authoring. **P1.1 `ReportSummary` now carries `needsReAuthoring`.** New `Option<String>` field on `ReportSummary` propagated via `From<&ReportDto> for ReportSummary`. The MCP `list_reports_needing_re_authoring` filter now actually filters — previously the field wasn't on the wire so it could never be non-null. **P1.2 needs-re-authoring banner in `ReportPage`.** The editor loads the empty stub but no longer presents it as a normal report. New state `needsReAuthoring`/`reAuthoringAccepted`; an Alert renders above the editor with the parse reason + an explicit "Re-author from scratch" confirm button. Editor area is locked behind that gate (test-id `reauthoring-editor-locked`); viewer area is locked unconditionally for needs-re-authoring reports (test-id `reauthoring-viewer-locked`); `canSave` requires `!needsReAuthoring || reAuthoringAccepted`. **P2.4 `/edit` + `ReportEditOp` exposed in OpenAPI.** `EditReportRequest` + `EditReportResponse` gained `utoipa::ToSchema` derives; `edit_report` handler gained `#[utoipa::path]`; `server.rs` `ApiDoc` paths/components updated to register `edit_report`, `ReportEditOp`, `BlockPosition`, `LayoutTarget`. FE codegen now surfaces them. New typed `editReport(token, { id, ops })` query helper + `useEditReport` React Query mutation hook expose the canonical `/edit` path to the FE. The wizard intentionally keeps PUT for full-replacement saves (its editing model is load-full/edit/save-full) — `/edit` is the targeted-partial path for MCP authoring flows + future per-block UIs, now fully discoverable. **P2.5 MCP authoring guidance updated.** `get_report_authoring_schema` no longer references the deleted per-op tools (`add_report_block`, `replace_report_block`, `add_report_layout_node`, …); `relatedTools` lists `edit_report` instead. The layout-guidance examples reflect the Phase 9 collapse: only `block` + `grid` (recursive) — no more `metric_row`/`section`/`columns` node types. Tool-group summary in `server.rs` rewritten to drop the ten deleted tool names. Row-condition examples for `visibleWhen`/`hiddenWhen`/`disabledWhen` updated from the legacy `{op, arguments: [field, value]}` shape to the canonical `ConditionExpression` form to match the typed Rust fields. **P3 per-source field-known checks moved into the provider trait.** New `ReportSourceProvider::markdown_field_known(block, field) -> bool` trait method handles dotted-path resolution + mode-dependent aggregate output aliases internally; `SystemProvider` overrides to branch on `Filter` vs `Aggregate` mode. Shared `dotted_field_known` helper in `providers.rs`. The two big inline blocks in `validate_report_definition` for `WorkflowRuntime` and `System` collapsed to one `matches!(...)` branch that goes through `provider.validate_block` + `provider.markdown_field_known`. The provider-internal helpers (`workflow_runtime_fields`/`workflow_runtime_row_field_known`/`system_fields`/`system_row_field_known`) are no longer reached from `ReportService` for this path. Verified: 395 server lib + 4 corpus + 3 proptest + 1 render + dsl Rust tests pass (lib +2 from the new repo unit tests); 577 vitest tests pass; 7 wizard-v2 mocked Playwright specs pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt` clean; `tsc -b` clean.

- 2026-05-18: Phase 11 — positional grid items + in-place block editing. UX gaps reported: (a) "I have a 4×4 grid. When I click 'Add block' in the slot, it always adds it to the first one, not to the one I clicked"; (b) "editing/creation of block should be in place — right now it's adding block, and expanding edit below. I'd prefer in-place edit to save space and less items". **DSL.** `ReportGridLayoutItem` gains optional `col`/`row` (1-indexed CSS-style). When set, renderers pin the item via `grid-column`/`grid-row` instead of letting CSS auto-flow place it. Items without `col`/`row` keep auto-flowing — backward-compatible. `LayoutTarget` mirrors the fields; `wrap_in_grid_item(node, col, row)` threads them onto items created via `AddLayoutNode`/`MoveLayoutNode`. Two new dsl unit tests cover add-with-col/row and move-to-explicit-cell. **Server validator.** `validate_layout_node` now also rejects: `col < 1`; `col + colSpan - 1 > columns`; `row < 1`; `row + rowSpan - 1 > rows` (when `rows` is declared); explicit-position overlap between any two items in the same grid. **FE walkers** ([layoutOps.ts](crates/runtara-server/frontend/src/features/reports/components/wizard-v2/layoutOps.ts)). `addLayoutNode` accepts `target.col`/`target.row` and sets them on the new `ReportGridLayoutItem`. `moveLayoutNode` honors them too (fast-path now also returns no-op when both source and target reference the same `(col, row)`). New `computeOccupiedCells(items, columns, rows)` simulates CSS placement — explicit-position items claim their cells first, then auto-flow items fill remaining cells in row-major order. `listEmptyCells(...)` returns ordered `(row, col)` empties inside the declared rectangle. **FE renderer** (both [GridContainer.tsx](crates/runtara-server/frontend/src/features/reports/components/wizard-v2/GridContainer.tsx) editor and [ReportRenderer.tsx](crates/runtara-server/frontend/src/features/reports/components/ReportRenderer.tsx) viewer). When `item.col` set: `grid-column: ${col} / span ${colSpan ?? 1}` (same for row). Empty-cell rendering walks `listEmptyCells` and assigns each placeholder explicit `grid-column`/`grid-row` so the user clicks the *exact* cell — fixing bug (a). Click handler is now `handleAddBlockToGrid(col, row)` which sets the new item's `col`/`row`. **In-place editor** (fixes bug b). New [InlineBlockEditor.tsx](crates/runtara-server/frontend/src/features/reports/components/wizard-v2/InlineBlockEditor.tsx): header (drag handle, "Editing <title>" label, type tag, Done button, delete) + body wrapping the existing `<BlockEditor>` form inside `max-h-[60vh] overflow-y-auto`. `BlockNodeEditor` rewritten with local `mode: 'preview' | 'edit'` state; preview hosts `<BlockHostInEdit>` only (no more expand-below), edit hosts `<InlineBlockEditor>`. Escape key dismisses. `BlockHostInEdit.onConfigure` renamed to `onEdit`; tooltip says "Edit block". `GridContainer` tracks recently-added block ids; the new block opens *directly* in edit mode (consumed once on mount). **DnD.** `EmptyCellPlaceholder` registers as a `useDroppable` with composite id `empty:<gridId>:<col>:<row>`. `resolveDrop` parses that id and returns a target with `col`/`row` set; `moveLayoutNode` pins the dragged item to the dropped cell. Hover state highlights the over-cell. **MCP authoring schema.** `layoutGuidance.currentContract` documents `col`/`row`; new `positionalCells4x4` example shows pinned items + spans in a 4×4 grid. **Tests.** 3 new dsl unit tests (add+col/row, move+col/row, default-no-position); 6 new layoutOps cases (positional add, no-position add, move-to-cell, computeOccupiedCells empty/explicit/mixed, listEmptyCells); 2 new dndResolve cases (drop on root-grid empty cell, drop on nested-grid empty cell); 2 new gridEditor cases (click bottom-right cell of 3×2 pins col=2 row=3, just-added block opens in edit mode) plus 1 new "renders empty placeholders only at unoccupied cells" case; new Playwright spec "clicking the bottom-right empty cell of a 3×2 grid pins the new block to that exact cell" that opens edit mode → Done → Save and asserts `col: 3, row: 2`. Verified: 50 dsl + 396 server lib + 4 corpus + 3 proptest + 1 render Rust tests pass; 593 vitest tests pass (was 581, +12); 14 wizard-v2 + corpus mocked Playwright specs pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all -- --check` clean; `tsc -b` clean; dev server boots without errors.

- 2026-05-18: Phase 10 — root must be a grid. UX gap reported: "right now we can add blocks directly to report root. this is not correct. Layout should start with grid, and then blocks be added to grid slots. Also, the whole layout root should be a grid (you need to have at least one grid)." Phase 9 had collapsed the layout vocabulary to block+grid but still allowed a `Vec<ReportLayoutNode>` at root, so reports could mix bare-block siblings with grids at the top level. Phase 10 makes the root a single mandatory grid. **DSL.** `ReportDefinition.layout: Vec<ReportLayoutNode>` → `ReportGridLayoutNode` (singular, required), and the same change applied to `ReportViewDefinition.layout`. New `runtara_report_dsl::types::default_root_grid()` returns the empty 1×1 root (`{ id: "root", columns: 1, items: [] }`); `#[serde(default = default_root_grid)]` keeps minimal wire payloads usable. **Migration.** Repository's `migrate_legacy_layout_in_definition` now also wraps the legacy `Vec`/array shape — every top-level node becomes an item of the new root grid (id="root", columns=1). Bare blocks at root get wrapped automatically. View layouts get their own per-view root grid (`view_<id>_root`). Pre-Phase-9 legacy containers (`section`/`columns`/`metric_row`) still migrate to grids first; the new wrapper kicks in last. Six new repo tests cover the wrapping + legacy-shape passthrough invariants. **Edit ops.** `LayoutTarget.parent_node_id` now resolves to the root grid when `None` (was: "the root Vec"); explicit `Some("root")` is also valid. New invariants: `RemoveLayoutNode` rejects the root id (`CANNOT_REMOVE_ROOT_GRID`), `MoveLayoutNode` rejects moving the root (`CANNOT_MOVE_ROOT_GRID`), `ReplaceLayoutNode` rejects replacing root with a block (`ROOT_LAYOUT_MUST_BE_GRID`), and `PatchLayoutNode` rejects type-changes on the root. Walkers (`find_in_grid_mut`/`replace_in_grid`/`remove_from_grid`/`insert_into_target`) now descend from the root grid instead of iterating a Vec. Five new dsl unit tests cover the invariants + default-root-grid serde fallback. **Server validator.** `validate_report_definition` builds a synthetic `{type: 'grid', ...}` wire-form value via `root_grid_as_layout_node_value` and runs the existing `validate_layout_node` once over the root (and again per view). The recursion handles all nested checks unchanged. **Corpus fixtures.** All 11 fixtures rewritten — `layout: [...]` arrays became `layout: { id: "root", columns: 1, items: [...] }` objects with the original nodes wrapped as items. Snapshots (`render_*.snap` / `runtime_validate_*.snap`) untouched because they never serialized layout. **MCP authoring guidance.** Schema text now explicitly states "definition.layout is a single mandatory root grid; every block must live inside its items[]". New `rootShape` example block + updated `layoutGuidance.layoutNodes` examples show the root grid wrapping nested grids. Big `examples.layout` rewritten as a root grid with three items (intro / summary 2-col / 12-col dashboard). View-layout examples updated. New lint warning `LEGACY_LAYOUT_ARRAY_SHAPE` fires when authors POST `layout: [...]` (transparently migrated server-side but flagged for visibility). **FE.** `types.ts` tightens `ReportDefinition.layout` to non-optional `ReportGridLayoutNode`. `utils.ts` walkers (`extractLayoutBlockReferences`, `getActiveReportLayout`, `extractVisibleLayoutBlockReferences`, `getEagerBlocks`) operate on the root grid. New `wizard-v2/layoutOps.ts`: walkers + `pathToLayoutNode`/`addLayoutNode`/`removeLayoutNode`/`moveLayoutNode`/`updateGrid`/`updateGridItem` all rooted at the single grid; root-grid protection in `removeLayoutNode`/`moveLayoutNode`; new `newDefaultLayout()` + `ROOT_GRID_ID` constant. `GridContainer` no longer maps a Vec — it renders the root grid directly via `<GridNodeEditor isRoot />`. The `isRoot` flag suppresses the drag handle and the "Remove grid" button on the root. `dndResolve.ts` walks from the root grid; the test for "container drop on root" works directly. `ReportRenderer` (viewer) renders the root grid's items uniformly. `ReportExplorePage.appendBlockToDefinition` adds new blocks as items of the root grid. **Tests.** `layoutOps.test.ts` rewritten with 21 cases covering the new shape + root protection. `dndResolve.test.ts` rewritten for nested-grid drop targeting. `gridEditor.test.tsx` updated to assert against the root grid directly. `losslessRoundTrip.test.tsx` now exercises `moveLayoutNode` in-place no-op (with new fast-path in layoutOps that returns unchanged when source position == target). `ReportPage.test.tsx` + `utils.test.ts` updated to construct definitions with a root grid. **Playwright.** `wizard-v2-grid-layout.mocked.spec.ts` rewritten: asserts the root grid is always visible on a new report, has no "Remove grid" affordance, and that columns/rows steppers operate on the root. `wizard-v2-author-flow.mocked.spec.ts` updated assertions to look for the new block as a root-grid item. `wizard-v2-grid-dnd.mocked.spec.ts` uses the new shape. `report-corpus-block-loading.mocked.spec.ts` + `report-builder-dsl-parity.mocked.spec.ts` auto-place each declared block as a root-grid item in their `baseDefinition` helpers. Verified: 581 vitest tests pass (was 577 → +4 for new layoutOps + root cases); 7 wizard-v2 mocked Playwright specs pass; 7 corpus mocked specs pass; 48 dsl unit tests pass; 396 server lib + 4 corpus + 3 proptest + 1 render Rust tests pass; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all -- --check` clean; `tsc -b` clean; dev server boots without errors. The pre-existing 36-failing parity-suite count is unchanged by this refactor (verified by stash-and-rerun).
