...
| 9 — Grid-only layout DSL + visual layout editor + in-place editing | [x] Done (drag-and-drop deferred to a follow-up). DSL collapsed to `Block` + `Grid` (recursive `items[].child`); legacy `section` / `columns` / `metric_row` types translated transparently by `parse_stored_definition`. Server validator + `apply_edit_ops` walker collapsed accordingly. Corpus fixtures rewritten + snapshots green. FE: `ReportRenderer` collapsed to one path; `GridContainer` + `GridSettingsPanel` + `BlockHostInEdit` replace the flat block list; preview wiring restored on `ReportPage` so blocks render live via `ReportBlockHost`. Two Playwright author-flow specs pass end-to-end including a 2-column grid round-trip. |
