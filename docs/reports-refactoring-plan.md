...
**Status:** Phase 0 complete (dual-run harness body deferred to Phase 1 by design; CI wiring intentionally out of scope)
**Owner:** _unassigned_
...
- [ ] Create `crates/runtara-report-dsl` crate.
- [ ] Move report-specific types out of `runtara-server`: layout, view, block, filter, dataset, source descriptors, slug/label helpers.
- [ ] Move the virtual aggregate engine (`aggregate_virtual_rows` + reducers + `evaluate_virtual_expression` at `services/reports.rs:8489–9131`) into the crate.
- [ ] Depend on `runtara-dsl` for `ConditionExpression`, `ConditionOperator`, `ConditionArgument`, `MappingValue`.
- [ ] Depend on `minijinja`. Expose `pub fn register_report_filters(env: &mut Environment)` for `currency`, `pill`, `datetime`, `bar_indicator`, etc.
- [ ] Add `evaluate_row_condition(expr: &ConditionExpression, row: &Value) -> Result<bool>`. Reject server-only operators (`MATCH`, `SIMILARITY_GTE`, `COSINE_DISTANCE_LTE`, `L2_DISTANCE_LTE`) with a structured error.
- [ ] Delete `validate_safe_display_template` family from `services/reports.rs:6244–6395`.
- [ ] Delete row-condition validator from `services/reports.rs:5713–5856`.
- [ ] Delete parallel template parser from `mcp/tools/reports.rs:3752–3839`.
- [ ] Add `wasm32-unknown-unknown` target + `wasm-bindgen` JS bindings.

### Tests

- [ ] Unit (Rust): minijinja conformance — every display-template fixture renders identically to the old custom parser output.
- [ ] Unit (Rust): viewer-template conformance — `{{user.id}}` / `{{user.email}}` / `{{user.method}}` work via minijinja.
- [ ] Unit (Rust): per-filter conformance (`currency`, `pill`, `datetime`, `bar_indicator`).
- [ ] Unit (Rust): `ConditionExpression` conformance — every row-condition fixture evaluates to the same boolean the old custom evaluator produced.
- [ ] Unit (Rust): re-home the 74 inline tests from `services/reports.rs` next to the moved code; all green.
- [ ] Unit (WASM): build WASM in CI; Node-side round-trip of `ReportDefinition` through `validate` + `evaluate_row_condition` + `render_template`.
- [ ] Integration (Rust): corpus snapshots unchanged vs Phase 0.
- [ ] Integration (CI): dual-run green.

### Acceptance

- [ ] Zero diff in dual-run.
- [ ] WASM bundle <250KB gzipped.
- [ ] `services/reports.rs` shorter by ~700 lines.
...
- [ ] Wire reports into existing `swagger-typescript-api` pipeline.
- [ ] Delete `frontend/src/features/reports/types.ts` (805 lines).
- [ ] Build `runtara-report-dsl` WASM bundle; publish to FE as `@runtara/report-dsl`.
- [ ] In `utils.ts`: replace `compileDisplayTemplate` (419–513) with minijinja-WASM call.
- [ ] In `utils.ts`: replace custom row-condition evaluator (267–403) with `evaluate_row_condition` WASM call.
- [ ] In `utils.ts`: delete `compareConditionValues`, `conditionValueOrdering`, and related helpers.
