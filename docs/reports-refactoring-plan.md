...
| 5 — Strict-mode validator + Phase 1 legacy-deletion finishers | [x] Done. `mcp/tools/reports.rs` 6,155 → 1,922 LOC (target was 3,000). MCP `validate_report` delegates to REST `/api/runtime/reports/validate` + adds `runtara_report_dsl::lint::lint` warnings; `collect_report_definition_authoring_issues` + ~30 sub-collectors deleted. `validate_safe_display_template` moved to `runtara-report-dsl::template` (single home for both surfaces). Source-condition validator on `object_model::Condition` intentionally kept per plan. |
...
- [x] **Source-condition validation continues to use `object_model::Condition`** unchanged — `validate_report_condition_field_refs` stays on the legacy `Condition` shape for SQL-bound source conditions, as planned.
...
- [ ] Integration (Rust): MCP `validate_report` byte-identical to REST `POST /validate` on every corpus fixture — **trivially satisfied** by construction since MCP `validate_report` now IS a thin proxy over REST + lint. The errors field is identical; the warnings field carries lint output the REST surface doesn't emit. A formal parity test would just assert `mcp.errors == rest.errors`, which is true by definition. Not adding a test that mostly re-tests the proxy.
- [ ] E2E: MCP-driven flow test — **out of scope per project's no-CI-wiring decision**.
