...
cross-service contracts between Auth0, smo-management and runtara. runtara
*enforces* these contracts; smo-management and the Auth0 Post-Login Action
*produce* the data they describe.
...
Ownership enforcement is live: the route gate lets `Own` through, and the handler
...
- This is an **enforced resource vocabulary** plus one UI-only capability
  ...
  enforced map. Consequently **Owner and Admin grant the same set** — the distinction
...
| Admin | `admin` | Same resource permissions as Owner (no member/billing/tenant split in the enforced map). |
...
Custom roles and granular per-user permissions are out of scope: the four
built-in roles are the whole vocabulary.

---

## 6. `audit_events` schema

Defined **once** here. Both runtara's local `audit_events` table and
smo-management's aggregated `audit_events` table MUST use this exact
...
test in runtara (`permission_map_matches_contract`) so that any drift between
this document and the code fails CI.
