...
**Slice a — LANDED (2026-07-15), emitter-fold path:** a pure workflow compiled with `WorkflowAbi::AgentCapabilities` exports `runtara:agent-<id>/capabilities.invoke(capability-id, input, connection) -> result<list<u8>, error-info>` — the exact agent shape — imports zero runtime, and is invocable AS an agent (verified via a wasmtime typed call returning the right payload). The local-index obstacle below was solved by refactoring the hand-assigned locals into `CANONICAL_LOCAL_GROUPS` + `drop_leading_locals(n)`: dropping the export's params off the FRONT keeps every surviving `DIRECT_*_LOCAL` at its absolute index and type for ANY param count (byte-identical for cli/invoke — 508 lib tests confirm). A second discovery: capabilities.invoke has 17 flat params (> the 16 flat-param limit), so the canonical ABI passes them INDIRECTLY — the single core param is a pointer to a params area (`cap-id`@0/4, `input`@8/12, `connection`@16), so the input is LOADED from `[params+8]`/`[params+12]`, not read from direct param slots. The non-suspending gate rejects any runtime-needing workflow (`E: not agent-eligible`). New host helper `WorkflowExecutor::invoke_capability` drives the typed call. **Next: slices b+c** — parameterize the export id to the workflow's own id (currently the fixed `workflow-agent`), stage the artifact as `runtara_agent_<id>.wasm`, and synthesize `meta.json` so it registers in the catalog and composes into a parent.
