...
Resolution (updated 2026-07-12): workflow execution and CRON static inputs now use the canonical shared `FormRenderer` backed by Rust/WASM schema normalization and validation; the interim duplicate schema renderer was deleted. CRON writes the same `{"data": {...}}` envelope as before. Its "Advanced (JSON)" toggle remains, and envelope keys the form cannot represent (`variables`, schema-unknown `data.*`, and other top-level keys) are surfaced in a warning and preserved verbatim on every structured edit.

Implementation detail: the trigger surface fetches the selected workflow schema, normalizes its supported legacy envelope into `FormDefinition`, and delegates field semantics to the same Rust/WASM engine used by workflow execution. Trigger-owned code continues to assemble and losslessly preserve the `{"data": {...}}` workflow input envelope; no trigger-to-workflow component import or duplicate schema renderer remains.
