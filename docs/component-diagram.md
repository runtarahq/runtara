...
flowchart LR
    ...
    subgraph server["runtara-server native process"]
        ...
        native["Native connection, outbound HTTP and database services"]
        ...
        agents -->|"Connection, HTTP and SQL imports"| hostIo
        hostIo -->|"Typed host imports"| native
        native --> connections
    ...
    providers["External APIs and object storage"]
    ...
    native -.->|"Authorized HTTP requests"| providers
...
flowchart LR
    ...
    agentSource["26 agent component crates"]
...
Arrows in this second view describe build inputs and selected library uses. Shared dependencies such as `runtara-ai`, encoding, and trusted execution are mapped below rather than repeating every edge.
...
| Component infrastructure | `component-host`, `agent-wit`, `workflow-wit`, `agent-macro`, `agent-bundle-emit`, `agent-trusted` | Wasmtime host, interface contracts, generated metadata glue, bundle metadata emitter, trusted-execution contract. Provider signing lives in the S3/Azure agents. |
| Host and domain support | `agents`, `connections`, `object-store`, `object-model-core`, `database-contract`, `report-dsl`, `text-parser`, `ai`, `agent-encoding` | Connection descriptors, OAuth/storage/rate limiting, tenant object storage, shared Object Model operations and SQL contracts, report schema/rendering, channel text parsing, AI provider support, shared text decoding. |
...
| Agent components: integrations | `agent-http`, `agent-mailgun`, `agent-slack`, `agent-teams`, `agent-openai`, `agent-ai-tools`, `agent-bedrock`, `agent-object-model`, `agent-s3-storage`, `agent-sqs`, `agent-azure-blob-storage`, `agent-sharepoint`, `agent-stripe`, `agent-hubspot`, `agent-quickbooks`, `agent-shopify`, `agent-mcp` | Dynamically selected capabilities; storage presigning uses approved isolated trusted execution. |
...
- [Native composition and dispatcher setup](../crates/runtara-server/src/server.rs), [trusted-executor wiring](../crates/runtara-server/src/server.rs), [embedded core/environment startup](../crates/runtara-server/src/embedded_runtara.rs).
- [Direct Environment client](../crates/runtara-server/src/runtime_client.rs), [execution engine](../crates/runtara-server/src/workers/execution_engine.rs), [durable admission lifecycle observer](../crates/runtara-server/src/workers/execution_outbox.rs).
