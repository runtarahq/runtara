...
**Remaining follow-ups (deliberate, gated):** Phase 6 retirement (runtime crate / workflow HTTP backend / CLI axis / guest-protocol router) — gated on N green releases, MUST NOT co-land with the flip, per §5. Store-freeing suspend optimizations (tri-state durable-sleep, wait `suspended(on-signal)` with the persisted deadline, custom-signal-insert waker, Delay sleep-key loop-index scoping) — semantics today are the legacy blocking behavior, preserved exactly; these are §3 Stage-2 optimizations, not correctness. Structured `error-info` mapping (v1 carries the raw error as `message`); CI drift-gate extension for the invoke WIT; neutral `runtara:abi` package minting.
...
> **Gate status (2026-07-15):** durable wait-result ✅ (`e8e663d9`), persisted wait deadline ✅ (`e8e663d9`), agent-retry per-attempt durability ✅ (`b1b28297`) — retry backoffs may now route through suspend. Still open: on-signal waker, sleep tri-state, Delay key scoping. **Corpus status:** `[wait → durable delay → finish]` ✅, `[wait → wait]` ✅ (shipped with fix A), `[retry-resume + per-iteration Split isolation]` ✅ (shipped with fix B). Still to add: `[split of N items each with a durable per-item delay]` (exercises the Delay key fix).
