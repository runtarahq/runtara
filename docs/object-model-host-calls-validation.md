...
| Exact generic SQL values | Eleven native driver tests include i64 limits, high-precision decimals, SQL NULL versus JSON null, date/time/UUID values, duplicate columns and empty row metadata. Legacy numeric presentation is isolated in the agent adapter. |
| Caller-owned SQL | SQLx sends the original SQL to PostgreSQL without an application parser or statement allowlist. Integration coverage exercises `DO`, `COMMENT`, `set_config`, and parameterized partial-index `ON CONFLICT ... WHERE ... DO UPDATE`. |
