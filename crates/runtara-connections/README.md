...
- Consumed exclusively by `runtara-server`, which mounts the three routers (`connections_router`, `admin_router`, `oauth_callback_router`) and holds an `Arc<ConnectionsFacade>` in `AppState`.
