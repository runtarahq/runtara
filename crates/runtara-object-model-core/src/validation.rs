...
impl SchemaValidator {
            ...
            } = &col.column_type
            {
                ...
                match src {
                    ...
                    Some(c) => match &c.column_type {
                        crate::types::ColumnType::String
                        | crate::types::ColumnType::Enum { .. } => {}
                        other => {
                            return Err(ValidationError::UnsupportedType(format!(
    ...
    fn validate_column(
        ...
        if let crate::types::ColumnType::Enum { values } = &col.column_type {
            if values.is_empty() {
        ...
        if let crate::types::ColumnType::Vector {
            dimension,
            index_method,
        } = &col.column_type
        {
            ...
            if let Some(crate::types::VectorIndexMethod::IvfFlat { lists }) = index_method
                && *lists == 0
            {
                return Err(ValidationError::UnsupportedType(format!(
        ...
        if matches!(col.text_index, crate::types::TextIndexKind::Trigram) {
            match &col.column_type {
                crate::types::ColumnType::String | crate::types::ColumnType::Enum { .. } => {}
                other => {
                    return Err(ValidationError::UnsupportedType(format!(
