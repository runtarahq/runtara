...
test.describe.serial('Workflow Lifecycle', () => {
  ...
  test('create a new workflow', async ({ request }) => {
  ...
  test('add a random-double step and save', async ({ request }) => {
    const res = await request.post(`${API_BASE}/${workflowId}/update`, {
      headers: apiHeaders(token),
  ...
  test('compile and activate version 2', async ({ request }) => {
    ...
    expect(workflow.data.currentVersionNumber).toBe(2);
  });
  ...
  test('compile and activate version 3', async ({ request }) => {
    ...
    // Compile
    const compileRes = await request.post(
      `${API_BASE}/${workflowId}/versions/3/compile`,
      { headers: apiHeaders(token) }
    );
    ...
    // Activate
    const activateRes = await request.post(
      `${API_BASE}/${workflowId}/versions/3/set-current`,
      { headers: apiHeaders(token) }
    );
    ...
    expect(getRes.status()).toBe(200);
    const workflow = await getRes.json();
    expect(workflow.data.currentVersionNumber).toBe(3);
  });
  ...
  test('execute and verify results', async ({ request }) => {
    ...
    const res = await request.post(
      `${GATEWAY_URL}/api/runtime/events/http-sync/${workflowId}`,
      {
        headers: apiHeaders(token),
        data: {},
      }
  ...
  test('clean up — delete workflow', async ({ request }) => {
