...
/**
 * Scenario Note Regression Test
 *
 * This test verifies that editing a note and saving the scenario
 * does not cause elements to disappear from the execution graph.
 *
 * Test steps:
 * 1. Open an existing scenario with multiple versions
 ...
 *
 * Note: Uses dynamic scenario selection from the scenarios list
 * to ensure the test works regardless of which scenarios exist.
 */

test.describe('Scenario Note Regression Tests', () => {
  ...
  test('editing a note should not cause graph elements to disappear', async ({
    ...
    // Step 1: Navigate to scenarios list and pick the first existing scenario
    ...
    // The skeleton shows gray rectangles, actual content will have scenario names as links
    await page.waitForLoadState('networkidle');

    // Wait for at least one scenario link to appear (not just the "New scenario" button)
    // Scenarios are displayed as cards/rows with links to /scenarios/{id}
    const scenarioCardSelector =
      'a[href^="/scenarios/"]:not([href="/scenarios/create"])';

    // Wait for either scenarios to load OR network error to appear
    const scenarioOrError = await Promise.race([
      page
        .waitForSelector(scenarioCardSelector, {
          state: 'visible',
          timeout: 30000,
        })
        .then(() => 'scenarios'),
    ...
    if (scenarioOrError === 'network-error') {
      await page.screenshot({
        path: 'e2e-results/screenshots/scenarios-list-network-error.png',
        fullPage: true,
      });
    ...
    if (scenarioOrError === 'timeout') {
      // Take a screenshot for debugging if no scenarios found
      await page.screenshot({
        path: 'e2e-results/screenshots/scenarios-list-debug.png',
        fullPage: true,
      });
      console.log('No scenarios found after waiting');
      test.skip(true, 'No existing scenarios found to test');
      return;
    }
    ...
    const scenarioLinks = page.locator(scenarioCardSelector);
    const scenarioCount = await scenarioLinks.count();
    console.log(`Found ${scenarioCount} scenario links`);
    ...
    // Wait for the scenario to load - check that the title is not "Untitled Scenario"
    // This ensures the actual scenario data has loaded
    ...
    const scenarioName = await page
      .locator('span.text-lg.font-semibold')
      .textContent();
    console.log(`Testing with scenario: ${scenarioName}`);

    // Step 2: Check if version selector exists (scenario has versions)
    ...
    if (hasVersions) {
      ...
      console.log('Scenario has no versions or version selector not visible');
    }
    ...
    await page.screenshot({
      path: `e2e-results/screenshots/scenario-note-regression-after-save.png`,
      fullPage: true,
    });
  });
  ...
  test('verify execution graph structure after note modification', async ({
    ...
    // Navigate to scenarios list and pick the first existing scenario
    await page.goto('/scenarios');
    await page.waitForLoadState('networkidle');
    ...
    const scenarioName = await page
      .locator('span.text-lg.font-semibold')
      .textContent();
    console.log(`Testing execution graph with scenario: ${scenarioName}`);

    // Intercept the API calls to capture the actual execution graph data
    let savedExecutionGraph: any = null;

    page.on('request', (request) => {
      if (request.method() === 'PUT' && request.url().includes('/scenarios/')) {
