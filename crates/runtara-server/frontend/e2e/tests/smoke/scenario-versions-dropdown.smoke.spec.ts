...
test.describe('Versions dropdown scroll', () => {
  test('versions dropdown should be scrollable with many versions', async ({
    ...
    // Wait a moment for scenarios to render
    await page.waitForTimeout(1000);

    // Find the Edit button on a scenario card to navigate to scenario editor
    ...
    // Check if version dropdown exists (scenario needs versions)
    const hasVersionDropdown = await versionButton
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    if (!hasVersionDropdown) {
      test.skip(true, 'Scenario does not have version dropdown');
      return;
    }
  ...
  test('version dropdown container has proper overflow styling', async ({
    ...
    // Find the Edit button on a scenario card
    const editButton = page.getByRole('button', { name: 'Edit' }).first();
    const hasEditButton = await editButton
      .isVisible({ timeout: 5000 })
      .catch(() => false);
