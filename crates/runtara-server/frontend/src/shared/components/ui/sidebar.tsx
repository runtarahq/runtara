...
// Component library pattern: exports hooks and context with components (shadcn/ui pattern)
...
>(
  (
    ...
    if (isMobile) {
      // Show minimized sidebar on mobile with Sheet overlay for full menu
      return (
        <>
          ...
          {/* Full sidebar as a sheet when toggled */}
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]:rounded-sm group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border/40 group-data-[variant=floating]:shadow-sm"
          >
            {children}
          </div>
        </div>
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      'mx-3 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border/60 px-2 py-0.5',
      'group-data-[collapsible=icon]:hidden',
      className
    )}
    {...props}
