...
export function InputMappingField(props: any) {
  ...
  const childWorkflowId = watch('childWorkflowId'); // Get childWorkflowId for EmbedWorkflow steps
  const isConditional = stepType === 'Conditional';
  const isSwitch = stepType === 'Switch';
  const isEmbedWorkflow = stepType === 'EmbedWorkflow';
  ...
  // Get input fields from child workflow's inputSchema for EmbedWorkflow steps
  // Must be called unconditionally to satisfy React hooks rules
  const childWorkflowInputFields = useMemo(() => {
    if (!isEmbedWorkflow || !childWorkflowId || !workflows) {
      return [];
    }

    // Find the child workflow
    const childWorkflow = workflows.find((s) => s.id === childWorkflowId);
    if (!childWorkflow?.inputSchema) {
      return [];
    }

    // Parse the inputSchema to get field definitions
    const schemaFields = parseSchema(childWorkflow.inputSchema);
  ...
  }, [isEmbedWorkflow, childWorkflowId, workflows]);
  ...
  // For EmbedWorkflow step types, use the SimpleInputMappingEditor with child workflow's input fields
  if (isEmbedWorkflow) {
    // If no child workflow selected yet, don't show the input mapping editor
    if (!childWorkflowId) {
      return null;
    }
    ...
    // If child workflow has no input fields, show a message
    if (childWorkflowInputFields.length === 0 && fieldArray.length === 0) {
      return (
        <div>
    ...
    return (
      <div>
        ...
        <SimpleInputMappingEditor
          nodeId={editorNodeId}
          fields={childWorkflowInputFields}
