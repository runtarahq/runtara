...
function getExecutionBadge(status?: ExecutionStatus) {
  switch (status) {
    ...
    case ExecutionStatus.Queued:
      return { label: status, variant: 'default' as const, icon: Circle };
...
function LaneContinuationNode({ node }: { node: Node }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 rounded-md border border-dashed bg-muted/30 px-3 py-2 text-sm">
...
function WorkflowTimelineItem({
  ...
  const statusVersion = useExecutionStore((state) => state.statusVersion);
  ...
  const status = useExecutionStore.getState().nodeExecutionStatus.get(node.id);
  const executionBadge = getExecutionBadge(status?.status);
  const ExecutionIcon = executionBadge?.icon;
  const hasValidationError = stepsWithErrors.has(node.id);
  const canInspectInDebug = !debugInspectMode || Boolean(status);
  ...
  // statusVersion is intentionally read to force status updates for getState() map lookups.
  void statusVersion;
        ...
        <div className="flex flex-col items-center">
          <button
            ...
            className={cn(
              ...
              hasValidationError && 'border-destructive text-destructive'
            )}
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            {executionBadge && ExecutionIcon && (
              <Badge variant={executionBadge.variant}>
                <ExecutionIcon className="mr-1 size-3" aria-hidden="true" />
