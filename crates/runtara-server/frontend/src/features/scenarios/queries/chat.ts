...
export async function createChatSession(
  ...
  const url = `${getRuntimeBaseUrl()}/scenarios/${encodeURIComponent(scenarioId)}/sessions`;
...
export async function fetchChatHistory(
  ...
  const url = `${getRuntimeBaseUrl()}/scenarios/${encodeURIComponent(scenarioId)}/instances/${encodeURIComponent(instanceId)}/step-events?sortOrder=asc&limit=1000`;
