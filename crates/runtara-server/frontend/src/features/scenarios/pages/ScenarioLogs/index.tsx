...
} from '@/features/scenarios/types/step-events';
...
export function ScenarioLogs() {
  ...
  const { data, isLoading } = useCustomQuery({
    queryKey: [
      ...queryKeys.scenarios.logs(instanceId ?? ''),
      scenarioId,
      pageIndex,
      pageSize,
      searchTerm,
  ...
  // Set page title
  usePageTitle(`Scenario Logs - Instance ${instanceId}`);
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                        ...
                        {log.scenarioName && (
                          <span>
                            Scenario:{' '}
                            {highlightText(log.scenarioName, searchTerm)}
                          </span>
                        )}
                      </div>
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {searchTerm || logLevelFilter !== 'all'
                ...
                : 'No logs available for this scenario instance'}
            </p>
          </div>
        )}
      </div>
