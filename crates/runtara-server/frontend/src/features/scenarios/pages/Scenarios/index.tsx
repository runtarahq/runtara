...
export function Scenarios() {
  ...
  // Get scenarios data for folder counts (uses recursive to get all scenarios)
  const { data: scenariosResponse } = useCustomQuery({
    queryKey: queryKeys.scenarios.all,
    queryFn: getScenarios,
  });
  const scenarios = useMemo(
    () => (scenariosResponse?.data?.content || []) as ScenarioDto[],
    [scenariosResponse?.data?.content]
  );

  // Count scenarios per folder
  const folderScenarioCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    scenarios.forEach((scenario) => {
      const folderPath = (scenario as any).path || '/';
  ...
  return (
    <TilesPage
      ...
      action={
        <Link to="/scenarios/create" className="w-full sm:w-auto">
          <Button
    ...
    >
          ...
          <div className="space-y-2 mb-6">
            {childFolders.map((folder, index) => (
              <FolderCard
                ...
                name={folder.name}
                path={folder.path}
                scenarioCount={folderScenarioCounts[folder.path] || 0}
      ...
      <DeleteFolderDialog
        ...
        scenarioCount={
          deleteFolderTarget ? folderScenarioCounts[deleteFolderTarget] || 0 : 0
        }
