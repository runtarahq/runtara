...
import { WorkflowEditor } from '@/features/scenarios/components/WorkflowEditor';
import { ScenarioActionsForm } from '@/features/scenarios/pages/Scenario/ScenarioActionsForm';
import { Loader } from '@/shared/components/loader.tsx';
import { composeExecutionGraph } from '@/features/scenarios/components/WorkflowEditor/CustomNodes/utils.tsx';
import { validateWorkflowStructure } from '@/features/scenarios/utils/graph-validation';
import '@xyflow/react/dist/base.css';
import { queryClient } from '@/main.tsx';
import { slugify } from '@/shared/utils/string-utils';
import { useEffect, useState, useRef, useMemo, useCallback } from 'react';
import * as form from '@/features/scenarios/components/WorkflowEditor/NodeForm/NodeFormItem';
import { useWorkflowStore } from '@/features/scenarios/stores/workflowStore.ts';
import { DebugStepInspector } from '@/features/scenarios/components/DebugStepInspector';

import { ScenarioExecuteDialog } from '@/features/scenarios/components/ScenarioExecuteDialog';
import {
  getScenarioVersions,
  getScenarioWorkflow,
  scheduleScenario,
...
import { ValidationPanel } from '@/features/scenarios/components/ValidationPanel';
import { useValidationStore } from '@/features/scenarios/stores/validationStore';
import {
  convertClientErrors,
  convertClientWarnings,
  convertServerErrors,
} from '@/features/scenarios/utils/validation-helpers';
...
export function Scenario() {
  ...
  // Staged scenario changes (name, description, variables, schemas, timeout) - preserved until global save
  const [stagedScenarioChanges, setStagedScenarioChanges] = useState<{
    name?: string;
    description?: string;
  ...
  // Compute if there are any unsaved changes (workflow dirty or staged node/scenario changes)
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    const hasScenarioChanges = Object.keys(stagedScenarioChanges).length > 0;
    return (
      isDirty || Object.keys(stagedNodeChanges).length > 0 || hasScenarioChanges
    );
  }, [isDirty, stagedNodeChanges, stagedScenarioChanges, isSwitchingVersion]);
  ...
  const hasStructuralUnsavedChanges = useMemo(() => {
    ...
    return (
      ...
      Object.keys(stagedScenarioChanges).length > 0
    );
  }, [
    isStructurallyDirty,
    stagedNodeChanges,
    stagedScenarioChanges,
    isSwitchingVersion,
  ]);
  ...
  const { data: versionsResponse } = useCustomQuery({
    ...
    queryFn: (token: string) => getScenarioVersions(token, scenarioId!),
    enabled: !!scenarioId,
  ...
  const { data: response, isFetching } = useCustomQuery({
    queryKey: queryKeys.scenarios.workflow(scenarioId ?? '', queryVersion),
    queryFn: (token: string) =>
      getScenarioWorkflow(token, scenarioId!, queryVersion),
    enabled: !!scenarioId,
    placeholderData: {
      data: {
        nodes: [],
  ...
  const scenarioInputSchema = useMemo(() => {
    const fields = (data as any)?.inputSchemaFields ?? [];
    return fields.length > 0 ? buildSchemaFromFields(fields) : {};
  }, [data]);

  // Reset selectedVersion and invalidate queries when scenarioId changes
  useEffect(() => {
    // Reset execution state when switching scenarios
    useExecutionStore.getState().resetExecution();

    // Reset workflow state when switching scenarios
    useWorkflowStore.getState().resetState();

    // Clear validation problems from previous scenario
    useValidationStore.getState().clearMessages();

    // Clear staged node and scenario changes
    setStagedNodeChanges({});
    setStagedScenarioChanges({});
    ...
    // Batch invalidate queries to ensure fresh data is fetched for the new scenario
    Promise.all([
      queryClient.invalidateQueries({
        queryKey: queryKeys.scenarios.workflow(scenarioId ?? ''),
      }),
      queryClient.invalidateQueries({
        queryKey: queryKeys.scenarios.versions(scenarioId ?? ''),
      }),
  ...
  // Reset execution state when leaving the scenario page
  useEffect(() => {
    return () => {
      useExecutionStore.getState().resetExecution();
    };
  ...
  }, [isFetching, scenarioId, queryVersion]);
  ...
  useEffect(() => {
    ...
    // API now provides trackEvents directly in ScenarioVersionInfoDto
  ...
  const performVersionChange = useCallback(
    (version: number | undefined) => {
      ...
      useWorkflowStore.getState().clearDirtyFlag();
      // Clear staged node and scenario changes when switching versions
      setStagedNodeChanges({});
      setStagedScenarioChanges({});
      ...
      queryClient.removeQueries({
        queryKey: queryKeys.scenarios.workflow(scenarioId ?? '', version),
        exact: true,
      });
  ...
  // Set page title with scenario name
  usePageTitle(data.name ? `Scenarios - ${data.name}` : 'Edit Scenario');

  // Track the staged scenario changes at mutation time to update cache correctly
  const stagedScenarioChangesRef = useRef(stagedScenarioChanges);
  useEffect(() => {
    stagedScenarioChangesRef.current = stagedScenarioChanges;
  }, [stagedScenarioChanges]);
  ...
  const updateMutation = useCustomMutation({
    ...
    onSuccess: async (response: any) => {
      // Response structure: { message, scenarioId, success, timestamp, version: "28" }
      ...
      const savedStagedChanges = stagedScenarioChangesRef.current;
      ...
      if (newVersionNumber) {
        ...
        const currentCachedData = queryClient.getQueryData(
          queryKeys.scenarios.workflow(scenarioId ?? '', queryVersion)
        ) as any;
        ...
        if (currentCachedData) {
          ...
          queryClient.setQueryData(
            queryKeys.scenarios.workflow(scenarioId ?? '', newVersionNumber),
            updatedData
          );
        }
      }
  ...
  const scheduleMutation = useCustomMutation({
    ...
    ) => scheduleScenario(token, scenarioId, inputs, version, debug),
    onSuccess: (
      response: any,
      variables: {
        scenarioId: string;
      ...
      // Response is wrapped: { data: ExecuteScenarioResponse, message, success }
      // ExecuteScenarioResponse: { instanceId, status }
      const instanceId = response?.data?.instanceId;

      if (instanceId && variables.scenarioId) {
  ...
  const { data: executionInstanceData, refetch: refetchInstanceData } =
    useCustomQuery({
      queryKey: queryKeys.scenarios.instance(
        scenarioId ?? '',
        executingInstanceId ?? ''
      ),
      queryFn: (token: string) =>
        getScenarioInstance(token, scenarioId!, executingInstanceId!),
  ...
  const { data: executionStepEventsData, refetch: refetchStepEvents } =
    useCustomQuery({
      queryKey: queryKeys.scenarios.stepEvents(
        scenarioId,
        executingInstanceId ?? undefined
      ),
      queryFn: (token: string) =>
        getStepEvents(token, scenarioId!, executingInstanceId!),
  ...
  useEffect(() => {
    ...
    if (!attachInstanceId || !scenarioId) return;
    ...
    startExecution(attachInstanceId, scenarioId, true, true);
  }, [
    searchParams,
    scenarioId,
    executingInstanceId,
  ...
  const setCurrentVersionMutation = useCustomMutation({
    ...
    onSuccess: async () => {
      // Remove all cached workflow versions for this scenario to ensure fresh data on return
      ...
      // Invalidate the main scenarios list to update the displayed version number
      await queryClient.invalidateQueries({
  ...
  const toggleTrackEventsMutation = useCustomMutation({
    ...
    onSuccess: async (
      response: any,
      variables: { scenarioId: string; version: number; trackEvents: boolean }
    ) => {
  ...
  const handleSubmit = async ({ name }: Record<string, any>) => {
    ...
    // Check if there are staged scenario changes (metadata: variables, schemas, description)
    const hasStagedScenarioChanges =
      Object.keys(stagedScenarioChanges).length > 0;
    ...
    if (!hasWorkflowChanges && !nameChanged && !hasStagedScenarioChanges) {
    ...
    const variablesArray =
      stagedScenarioChanges.variables ?? data.variables ?? [];
    ...
    const inputSchemaFieldsToUse =
      stagedScenarioChanges.inputSchemaFields ?? data.inputSchemaFields ?? [];
    const outputSchemaFieldsToUse =
      stagedScenarioChanges.outputSchemaFields ?? data.outputSchemaFields ?? [];
    ...
    const scenarioName = stagedScenarioChanges.name || data.name || '';
    // Description is optional - only include if it has a value
    const scenarioDescription =
      stagedScenarioChanges.description ?? data.description ?? '';
  ...
  const handleExecuteScenario = async (inputs: Record<string, any>) => {
    if (!scenarioId) {
      toast.error('Scenario ID is missing');
      return;
    }
  ...
  const handleExportJSON = () => {
    ...
    if (executionGraph) {
      ...
      // Use slugified scenario name and append version to the filename
  ...
  const performImportJSON = useCallback((jsonString: string) => {
    try {
      ...
      if (
        ...
        // Stage the scenario changes (variables, schemas, timeout)
        const scenarioChanges: typeof stagedScenarioChanges = {};
        if (variables.length > 0) {
          scenarioChanges.variables = variables;
        }
        if (inputSchemaFields.length > 0) {
          scenarioChanges.inputSchemaFields = inputSchemaFields;
        }
        if (outputSchemaFields.length > 0) {
          scenarioChanges.outputSchemaFields = outputSchemaFields;
        }
        if (executionTimeoutSeconds !== undefined) {
          scenarioChanges.executionTimeoutSeconds = executionTimeoutSeconds;
        }

        setStagedScenarioChanges(scenarioChanges);
  ...
  // Memoize scenario object to prevent unnecessary re-renders in ScenarioPropertiesDialog
  const scenarioForSettings = useMemo(
    () => ({
      id: scenarioId || '',
      name: stagedScenarioChanges.name ?? data.name ?? '',
      description: stagedScenarioChanges.description ?? data.description ?? '',
      variables: stagedScenarioChanges.variables ?? data.variables ?? [],
      inputSchemaFields:
        stagedScenarioChanges.inputSchemaFields ?? data.inputSchemaFields ?? [],
      outputSchemaFields:
        stagedScenarioChanges.outputSchemaFields ??
    ...
    [
      scenarioId,
      stagedScenarioChanges.name,
      stagedScenarioChanges.description,
      stagedScenarioChanges.variables,
      stagedScenarioChanges.inputSchemaFields,
      stagedScenarioChanges.outputSchemaFields,
      stagedScenarioChanges.executionTimeoutSeconds,
      stagedScenarioChanges.rateLimitBudgetMs,
      data.name,
      data.description,
      data.variables,
  ...
  if (!scenarioId) {
    return <div className="p-6 text-center">Scenario ID is missing</div>;
  }
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      <div className="relative flex-1 min-h-0 overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex justify-center">
          <div className="pointer-events-auto">
            <ScenarioActionsForm
              isLoading={isLoading}
              scenarioName={stagedScenarioChanges.name ?? data.name ?? ''}
      ...
      <ValidationPanel
        ...
        scenario={scenarioForSettings}
        onScenarioChange={(changes) => {
          setStagedScenarioChanges((prev) => ({
            ...prev,
            ...changes,
          }));
        }}
      ...
      {/* Debug execute dialog (server-side breakpoints) — reuses ScenarioExecuteDialog */}
      <ScenarioExecuteDialog
