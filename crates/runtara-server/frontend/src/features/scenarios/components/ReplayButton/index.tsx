...
import { replayScenario } from '@/features/scenarios/queries';
...
export function ReplayButton(props: Props) {
  ...
  const handleClick = async () => {
    ...
    setIsLoading(true);
    try {
      await replayScenario(token, instanceId);
      toast({
        title: 'Success',
        description: isTransient
          ? 'Scenario retry has been scheduled'
          : 'Scenario has been scheduled for replay',
      });
    } catch (error) {
      console.error('Error replaying scenario:', error);
      toast({
        title: 'Error',
        description: 'Failed to replay scenario',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
