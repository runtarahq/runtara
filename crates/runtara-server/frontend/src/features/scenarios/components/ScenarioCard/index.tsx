...
interface ScenarioCardProps {
  ...
  onSchedule: (scenario: ScenarioDto) => void;
  onClone: (scenario: ScenarioDto) => void;
  onChat?: (scenario: ScenarioDto) => void;
  onMoveToFolder?: (scenario: ScenarioDto) => void;
...
export function ScenarioCard({
  ...
  const version = scenario.currentVersionNumber;
  const isPendingThisCard = pendingActionId === scenario.id;
  ...
  const metadata = [
    scenario.updated ? (
      <>
        ...
        {getRelativeTime(scenario.updated)}
      </>
    ) : null,
    scenario.created ? (
      <>
        <Calendar className="w-3.5 h-3.5" />
        {formatDate(scenario.created)}
      </>
    ) : null,
        ...
        <>
          ...
          {showMoveAction && onMoveToFolder && (
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onMoveToFolder(scenario)}
