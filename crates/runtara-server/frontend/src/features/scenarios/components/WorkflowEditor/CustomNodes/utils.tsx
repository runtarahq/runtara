...
} from '@/features/scenarios/utils/start-step-migration';
...
export interface ExecutionGraph {
  ...
  entryPoint: string;
  notes?: Note[];
  // Static variables defined at scenario version level
...
function cleanNodeData(steps: Record<string, any>) {
  ...
  for (const [id, step] of Object.entries(steps)) {
    ...
    // Ensure StartScenario has childScenarioId and childVersion at root level (DSL v2.0.0 requirement)
    if (restData.stepType === 'StartScenario') {
      if (data.childScenarioId) {
        cleaned[id].childScenarioId = data.childScenarioId;
      }
  ...
  for (const [id, step] of Object.entries(steps)) {
    ...
    const node: Node = {
      ...
      data: {
        ...
        // Fix potentially double-stringified values for StartScenario steps
        ...(extendedData.childScenarioId && {
          childScenarioId: safeParseValue(extendedData.childScenarioId),
        }),
