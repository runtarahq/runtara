...
} from '@/features/scenarios/stores/executionStore';
...
>(
  (
    ...
    const handleClick = (e: MouseEvent<HTMLDivElement>) => {
      ...
      if ((e.ctrlKey || e.metaKey) && stepType === 'StartScenario') {
        // Navigate to the scenario editing page
        // The scenario ID is stored as "scenarioId" in input mapping
        if (inputMapping && inputMapping.length > 0) {
          const scenarioIdMapping = inputMapping.find(
            (item) => item.type === 'scenarioId'
          );
          if (
            scenarioIdMapping &&
            scenarioIdMapping.value &&
            typeof scenarioIdMapping.value === 'string'
          ) {
            const scenarioId = JSON.parse(scenarioIdMapping.value);
