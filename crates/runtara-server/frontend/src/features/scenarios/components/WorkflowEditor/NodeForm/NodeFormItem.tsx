...
import { StartScenarioConfigField } from './StartScenarioConfigField';
...
// Wrapper component for StartScenarioConfigField that uses react-hook-form
function StartScenarioFieldRenderer() {
  const form = useFormContext();
  const stepType = useWatch({ name: 'stepType', control: form.control });
  const childScenarioId = useWatch({
    name: 'childScenarioId',
  ...
  const handleScenarioIdChange = useCallback(
    (value: string) => {
      form.setValue('childScenarioId', value, {
        shouldValidate: true,
        shouldDirty: true,
        shouldTouch: true,
      });
      // Clear stale input mappings from the previously selected child scenario
  ...
  // Only show for StartScenario steps
  if (!stepType || stepType !== 'StartScenario') {
    return null;
  }

  return (
    <StartScenarioConfigField
      scenarioIdValue={childScenarioId || ''}
      versionValue={childVersion || ''}
      onScenarioIdChange={handleScenarioIdChange}
...
const basicFieldsConfig = [
...

  {
    ...
    initialValue: '',
    colSpan: 'full',
    type: 'hidden', // Field value, actual UI is rendered by startScenarioConfig
  },
  {
    label: '',
    name: 'childVersion',
  ...
  {
    label: '',
    name: 'startScenarioConfig',
    initialValue: undefined,
    colSpan: 'full',
    renderFormField: () => <StartScenarioFieldRenderer />,
  },
  {
    label: '',
    name: 'connectionId',
...
export const schema = (context: { agents: ExtendedAgent[] }) =>
  z
    .object({
      ...
      childScenarioId: z.string().optional(),
      childVersion: z.string().optional(),
      startScenarioConfig: z.any().optional(), // UI-only field
    ...
    .refine(
      (inputs) => {
        ...
        // Skip inputMapping validation for non-Agent steps (e.g., StartScenario)
    ...
    .refine(
      (inputs) => {
        const { stepType, childScenarioId } = inputs;
        if (stepType === 'StartScenario' && !childScenarioId) return false;
        return true;
      },
      {
        message: 'Child Scenario is required for StartScenario step.',
        path: ['childScenarioId'],
      }
    )
    .refine(
      (inputs) => {
        const { stepType, childVersion } = inputs;
        if (stepType === 'StartScenario' && !childVersion) return false;
        return true;
      },
      {
        message: 'Version selection is required for StartScenario step.',
        path: ['childVersion'],
      }
    )
    .refine(
