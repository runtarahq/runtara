...
import { useHierarchicalTimeline } from '@/features/scenarios/hooks/useHierarchicalTimeline';
import { HierarchicalStep } from '@/features/scenarios/types/timeline';
...
export function ExecutionTimeline({
  ...
  const {
    ...
    loadMoreRoot,
    toggleExpand,
  } = useHierarchicalTimeline(scenarioId, instanceId);
  ...
  const { data: pendingInputData } = useCustomQuery<PendingInput[]>({
    queryKey: queryKeys.scenarios.pendingInput(scenarioId, instanceId),
    queryFn: (token: string) => getPendingInput(token, scenarioId, instanceId),
    enabled:
      !!scenarioId && !!instanceId && isActiveStatus(instanceData?.status),
    refetchInterval: () => {
  ...
  // Reset local state when scenario or instance changes
  useEffect(() => {
  ...
  if (visibleSteps.length === 0) {
    return (
      <div className="py-16 px-6 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          Timeline events will appear here as your scenario executes. If your
          scenario is still running, events may appear soon.
        </p>
      </div>
    );
  }
