 ...
 */
export function HistoryPanelContent({ scenarioId }: HistoryPanelContentProps) {
  ...
  const { data: stepSummariesData, isLoading: stepsLoading } = useCustomQuery({
    ...
    queryFn: (token: string) =>
      getStepSummaries(token, scenarioId, selectedInstanceId!, stepFilters),
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center text-muted-foreground">
        ...
        <p className="text-xs">Run the scenario to see history here</p>
      </div>
    );
  }
    ...
    <div className="flex flex-1 min-h-0 overflow-hidden">
      ...
      <div className="w-72 border-r flex-shrink-0 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-3 py-1.5 border-b bg-muted/20">
          ...
          <Link
            to={`/invocation-history?scenarioId=${scenarioId}`}
