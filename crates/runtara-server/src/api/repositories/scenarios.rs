...
use crate::api::dto::scenarios::{Note, ScenarioDto, ScenarioVersionInfoDto};
...
impl ScenarioRepository {
    /// Create a new ScenarioRepository
    ...
    /// Create a new scenario metadata entry
    /// Returns (created_at, updated_at) timestamps
    /// Note: name/description are now stored in the execution graph, not in the scenarios table
    pub async fn create(
        &self,
        tenant_id: &str,
        scenario_id: &str,
        ...
        let row = sqlx::query!(
            r#"
            INSERT INTO scenarios (tenant_id, scenario_id, version_count, latest_version)
            VALUES ($1, $2, 0, 0)
    ...
    /// Create an initial version (version 1) for a new scenario with an empty graph
    ...
    pub async fn create_initial_version(
        ...
        sqlx::query(
            r#"
            INSERT INTO scenario_definitions (tenant_id, scenario_id, version, definition, file_size, memory_tier, track_events)
    ...
    /// Get the current (active) version number for a scenario
    ...
    /// Check if a scenario exists (not soft-deleted)
    pub async fn exists(&self, tenant_id: &str, scenario_id: &str) -> Result<bool, sqlx::Error> {
        let row = sqlx::query!(
            r#"
            SELECT 1 as "exists!"
            FROM scenarios
    ...
    /// Soft delete a scenario (marks as deleted, does not remove from database)
    /// Returns the number of rows affected
    pub async fn delete(&self, tenant_id: &str, scenario_id: &str) -> Result<u64, sqlx::Error> {
        // Use a transaction to ensure atomicity - both tables must be updated together
        let mut tx = self.pool.begin().await?;

        // First, mark all scenario definitions as deleted
        sqlx::query!(
            r#"
        ...
        // Then mark the scenario metadata as deleted
    ...
    /// List all scenarios (metadata only) with pagination and optional folder filtering
    /// Returns (list of scenarios, total count)
    /// Note: name/description are extracted from the execution graph (definition)
    ///
    /// # Arguments
    /// * `path` - Optional folder path filter. If None, returns all scenarios (backward compatible).
    ...
    pub async fn list(
        ...
        recursive: bool,
        search: Option<&str>,
    ) -> Result<(Vec<ScenarioDto>, i64), sqlx::Error> {
        ...
        // Build search filter — searches scenario name in the definition JSON
        ...
        let count_join = if search_value.is_some() {
            " LEFT JOIN scenario_definitions sd ON s.tenant_id = sd.tenant_id AND s.scenario_id = sd.scenario_id AND COALESCE(s.current_version, s.latest_version) = sd.version"
        } else {
            ""
        };
        ...
        let main_query = format!(
            r#"
            SELECT s.scenario_id, s.latest_version, s.current_version, s.created_at, s.updated_at,
                   sd.memory_tier, sd.track_events,
                   sd.definition,
                   s.path
            FROM scenarios s
            LEFT JOIN scenario_definitions sd ON s.tenant_id = sd.tenant_id
                AND s.scenario_id = sd.scenario_id
    ...
    pub async fn get_by_id(
        ...
        scenario_id: &str,
        version: Option<i32>,
    ) -> Result<Option<ScenarioDto>, sqlx::Error> {
        ...
        let version_to_fetch = if let Some(v) = version {
            ...
            {
                Some(v) => v,
                None => return Ok(None), // Scenario not found
            }
        };

        // Query scenario definition and metadata
        // The definition column in scenario_definitions IS the execution_graph
        ...
        let row: Option<(
            ...
            JOIN scenarios s ON sd.tenant_id = s.tenant_id AND sd.scenario_id = s.scenario_id
            WHERE sd.tenant_id = $1 AND sd.scenario_id = $2 AND sd.version = $3
    ...
    // ============================================================================
    // Scenario Definition (Version) Operations
    // ============================================================================
    ...
    pub async fn create_version(
        ...
        sqlx::query!(
            r#"
            INSERT INTO scenario_definitions (tenant_id, scenario_id, version, definition, file_size)
            VALUES ($1, $2, $3, $4, $5)
            "#,
            tenant_id,
            scenario_id,
    ...
    /// Get a specific version of a scenario definition
    ...
    /// Get scenario definition and track-events mode.
    ...
    /// Get scenario definition, memory tier, and track-events mode.
    ...
    /// List all versions of a scenario with compilation status
    pub async fn list_versions(
        &self,
        tenant_id: &str,
        scenario_id: &str,
    ) -> Result<Vec<ScenarioVersionInfoDto>, sqlx::Error> {
        let rows: Vec<ScenarioVersionRow> = sqlx::query_as(
            r#"
            SELECT
                sd.version,
                sd.created_at,
                ...
                s.latest_version
            FROM scenario_definitions sd
            LEFT JOIN scenario_compilations sc
                ON sd.tenant_id = sc.tenant_id
                AND sd.scenario_id = sc.scenario_id
                AND sd.version = sc.version
                AND sc.compilation_status = 'success'
            JOIN scenarios s
                ON sd.tenant_id = s.tenant_id
                AND sd.scenario_id = s.scenario_id
            WHERE sd.tenant_id = $1
                AND sd.scenario_id = $2
        ...
        let versions = rows
            ...
            .map(|row| {
                ...
                ScenarioVersionInfoDto {
                    scenario_id: scenario_id.to_string(),
                    version_id: format!("{}-v{}", scenario_id, row.0),
    ...
    /// Update the path (folder) of a scenario
    ...
    /// Rename a folder by updating all scenario paths that start with the old path
    /// Returns the number of scenarios updated
    ...
    /// Clone a scenario with all its versions to a new scenario ID
    ...
    pub async fn clone(
        ...
        // Fetch all versions of the source scenario
        ...
        // Create the new scenario metadata
    ...
    ///
    /// Stores binary path, checksum, and metadata in scenario_compilations table.
    ...
    ///
    /// Upserts the scenario_compilations table with the image_id returned
    ...
    /// Note: name/description are now stored in the execution graph (definition), not in the scenarios table.
    /// Returns the new version number.
    pub async fn update_scenario(
        ...
        let current: (i32, Option<String>, Option<bool>) = sqlx::query_as(
            ...
            SELECT (COALESCE(MAX(version), 0) + 1)::INT as next_version,
                   (SELECT memory_tier FROM scenario_definitions
                    WHERE tenant_id = $1 AND scenario_id = $2
                    ORDER BY version DESC LIMIT 1) as current_memory_tier,
                   (SELECT track_events FROM scenario_definitions
                    WHERE tenant_id = $1 AND scenario_id = $2
        ...
        // Update scenarios table metadata (only version info, name/description are in execution graph)
        sqlx::query!(
            r#"
            UPDATE scenarios
            SET latest_version = $1,
                ...
                updated_at = NOW()
            WHERE tenant_id = $2 AND scenario_id = $3
            "#,
            next_version,
            tenant_id,
            scenario_id
    ...
    /// Update track-events mode for a specific scenario version.
    ...
    pub async fn update_track_events(
        ...
        let current_track_events = self
            .get_track_events(tenant_id, scenario_id, version)
            .await?
            .unwrap_or(false);

        // Update track_events column in scenario_definitions
        sqlx::query(
            r#"
            UPDATE scenario_definitions
        ...
        if current_track_events != track_events {
            self.invalidate_compilation(tenant_id, scenario_id, version)
                .await?;
        }
    ...
    /// Invalidate (delete) the compiled binary for a scenario version
    ...
    pub async fn invalidate_compilation(
        ...
        sqlx::query!(
            r#"
            DELETE FROM scenario_compilations
            WHERE tenant_id = $1 AND scenario_id = $2 AND version = $3
            "#,
            tenant_id,
            scenario_id,
    ...
    /// This forces recompilation of all scenarios on next execution
    ...
    /// Get track-events mode for a specific scenario version.
    ...
    /// Soft delete a scenario and all its versions
    ///
    /// Marks the scenario and all its definitions as deleted.
    ...
    /// Clone a scenario to a new scenario ID with a new name
    ///
    /// Copies only the current version (or latest if current not set) to a new scenario.
    /// The cloned scenario starts at version 1.
    ...
    pub async fn clone_scenario(
        ...
        let version_to_clone = match self
            .get_current_or_latest_version(tenant_id, source_scenario_id)
            .await?
        {
            Some(v) => v,
            None => return Ok(0), // Source scenario not found
        };
        ...
        // Create new scenario metadata WITHOUT current_version first
        ...
        sqlx::query(
            ...
            INSERT INTO scenario_definitions (
                tenant_id, scenario_id, version, definition, file_size, memory_tier, track_events, created_at
            )
            VALUES ($1, $2, 1, $3, $4, $5, $6, NOW())
            "#,
        )
        .bind(tenant_id)
        .bind(new_scenario_id)
    ...
    pub async fn set_current_version(
        ...
        sqlx::query!(
            ...
            SET current_version = $1,
                updated_at = NOW()
            WHERE tenant_id = $2 AND scenario_id = $3 AND deleted_at IS NULL
            "#,
            version_number,
            tenant_id,
            scenario_id
    ...
    // ============================================================================
    // Scenario Dependency Operations
    // ============================================================================

    /// Record a dependency between parent and child scenarios
    ...
    /// Get all dependencies for a parent scenario (all versions or specific version)
    ...
    /// Get all parent scenarios that depend on a child scenario
    pub async fn get_dependents(
        ...
        let rows = sqlx::query!(
            r#"
            SELECT parent_scenario_id, parent_version, child_version_resolved, step_id
            FROM scenario_dependencies
            WHERE parent_tenant_id = $1
                AND child_scenario_id = $2
                AND ($3::INTEGER IS NULL OR child_version_resolved = $3)
            ORDER BY parent_scenario_id, parent_version DESC
            "#,
            tenant_id,
            child_scenario_id,
            child_version
    ...
    pub async fn get_version_schemas(
        ...
        let row: Option<(Value,)> = sqlx::query_as(
            ...
            FROM scenario_definitions sd
            JOIN scenarios s ON s.tenant_id = sd.tenant_id AND s.scenario_id = sd.scenario_id
            WHERE sd.tenant_id = $1 AND sd.scenario_id = $2 AND sd.version = $3
            AND sd.deleted_at IS NULL AND s.deleted_at IS NULL
            "#,
        )
    ...
    /// Get execution timeout for a scenario version from the executionGraph
    ...
    /// Get scenario names for multiple scenario IDs in bulk
    ///
    /// Returns a HashMap mapping scenario_id -> (name, current_version).
    /// This is used to enrich execution listings with scenario metadata.
    /// The name is extracted from the execution graph of the current (or latest) version.
    #[allow(clippy::type_complexity)]
    pub async fn get_scenario_names_bulk(
        &self,
        tenant_id: &str,
        scenario_ids: &[String],
        ...
        // Query scenario names from the execution graph (definition)
        // Join with scenario_definitions to get the name from current/latest version
        let rows: Vec<(String, Option<Value>, Option<i32>, Option<i32>)> = sqlx::query_as(
            r#"
            SELECT s.scenario_id, sd.definition, s.current_version, s.latest_version
            FROM scenarios s
            LEFT JOIN scenario_definitions sd ON s.tenant_id = sd.tenant_id
                AND s.scenario_id = sd.scenario_id
                AND COALESCE(s.current_version, s.latest_version) = sd.version
                AND sd.deleted_at IS NULL
            WHERE s.tenant_id = $1 AND s.scenario_id = ANY($2) AND s.deleted_at IS NULL
            "#,
        )
        .bind(tenant_id)
        .bind(scenario_ids)
        ...
        for (scenario_id, definition, current_version, latest_version) in rows {
            let name = definition
                .as_ref()
            ...
            result.insert(scenario_id, (name, version));
        }

        Ok(result)
    }

    /// Look up scenario info by registered image IDs (UUIDs from runtara-environment)
    ///
    /// Returns a HashMap mapping registered_image_id -> (scenario_id, version, name).
    /// This is used to enrich execution listings when we only have the image UUID from Runtara.
    pub async fn get_scenario_info_by_image_ids(
        &self,
        tenant_id: &str,
        image_ids: &[String],
        ...
        // Query scenario info by registered_image_id
        // Join with scenario_definitions to get the scenario name from the definition
        let rows: Vec<(String, String, i32, Option<Value>)> = sqlx::query_as(
            r#"
            SELECT sc.registered_image_id, sc.scenario_id, sc.version, sd.definition
            FROM scenario_compilations sc
            JOIN scenario_definitions sd ON sc.tenant_id = sd.tenant_id
                AND sc.scenario_id = sd.scenario_id
        ...
        let mut result = std::collections::HashMap::new();
        for (image_id, scenario_id, version, definition) in rows {
            let name = definition
                .as_ref()
                .and_then(|d| d.get("name"))
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string();
            result.insert(image_id, (scenario_id, version, name));
        }
