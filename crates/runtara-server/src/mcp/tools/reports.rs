        ...
        "blockShape": {
            ...
            "card": {
                ...
                "fieldShape": {
                    ...
                    "workflowAction": "Optional card field button: set kind='workflow_button' and workflowAction={id, workflowId, version?, label?, runningLabel?, successMessage?, reloadBlock?, visibleWhen?, hiddenWhen?, disabledWhen?, context?}. id must be stable and unique within the block. context.mode is row | field | value, and context.inputKey can wrap the selected context into an object. visibleWhen/hiddenWhen/disabledWhen are ConditionExpression objects (same shape used for workflow step conditions) evaluated against the rendered row.",
