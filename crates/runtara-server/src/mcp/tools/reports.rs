...
fn report_authoring_schema() -> Value {
    let mut result: Value = serde_json::from_str(r###"{
        ...
        "definitionShape": {
            ...
            "viewGroups": "Optional sibling navigation over named views. mode='tabs' uses ordered viewIds. mode='stages' uses ordered {viewId,value} stages plus currentFrom={type:'filter',filterId} or {type:'block',blockId,field}; access='through_current' keeps future stages unavailable, while access='current_only' requires a non-lazy block field, exposes only that field-selected stage, and prevents stage switching. showPrevious/showNext independently control the stage buttons, previousLabel/nextLabel customize their copy, and legacy showPreviousNext remains supported. followCurrentOnAdvance controls post-action navigation.",
        ...
        "biGuidance": {
            "currentContract": [
                ...
                "For ordered workflows, define a viewGroups entry with mode='stages', ordered stages, currentFrom, and access='through_current'. Use access='current_only' for a read-only, field-controlled stage rail. Use a non-lazy block currentFrom when the persisted stage lives on the selected record.",
        ...
        "blockShape": {
            ...
            "table": {
                ...
                "actions": [{"id": "bulk_process", "label": "Process selected", "workflowAction": {"id": "bulk_process", "workflowId": "process_items", "label": "Process selected", "runningLabel": "Processing...", "successMessage": "Selected rows processed.", "reloadBlock": true, "context": {"mode": "selection", "inputKey": "items"}}}],
                "writeback": {
                ...
                "workflowAction": "Optional table column button: set type='workflow_button' and workflowAction={id, workflowId, version?, label?, runningLabel?, successMessage?, reloadBlock?, visibleWhen?, hiddenWhen?, disabledWhen?, context?}. id must be stable and unique within the block. context.mode is row | field | value. mode=row passes the whole row as workflow data; mode=field passes context.field or column.field; mode=value passes the cell value. context.inputKey wraps the context as {inputKey: context}. visibleWhen/hiddenWhen/disabledWhen are ConditionExpression objects evaluated against the rendered row, e.g. disabledWhen={type:'operation', op:'EQ', arguments:[{valueType:'reference', value:'status'}, {valueType:'immediate', value:'processed'}]}. The flat {op, arguments} shape used by source.condition is also accepted here and normalized to the ConditionExpression form on save, so the same disabledWhen can be written as {op:'EQ', arguments:['status','processed']}.",
                "interactionButtons": "Optional row navigation/action buttons: set type='interaction_buttons' and interactionButtons=[{id,label?,icon?,visibleWhen?,hiddenWhen?,disabledWhen?,actions:[...]}]. Button actions use the same set_filter, clear_filter, clear_filters, and navigate_view vocabulary as block.interactions. visibleWhen/hiddenWhen/disabledWhen accept either condition shape — the tagged ConditionExpression form or the flat {op, arguments} form used by source.condition — and are normalized to ConditionExpression on save. Use this for rows like SKU | Qty | Price | View 1 | View 2.",
            ...
            "card": {
                ...
                "fieldShape": {
                    ...
                    "workflowAction": "Optional card field button: set kind='workflow_button' and workflowAction={id, workflowId, version?, label?, runningLabel?, successMessage?, reloadBlock?, visibleWhen?, hiddenWhen?, disabledWhen?, context?}. id must be stable and unique within the block. context.mode is row | field | value, and context.inputKey can wrap the selected context into an object. visibleWhen/hiddenWhen/disabledWhen are ConditionExpression objects (same shape used for workflow step conditions) evaluated against the rendered row; the flat {op, arguments} shape used by source.condition is also accepted and normalized on save.",
        ...
        "sourceShape": {
            ...
            "condition": "Optional condition DSL. Written flat and positionally: {op, arguments} with the field name as the first operand, e.g. {op:'EQ', arguments:['status','active']}. The tagged ConditionExpression shape used by visibleWhen/hiddenWhen/disabledWhen is also accepted here and normalized to the flat form on save, so you can write either. Object Model sources can use schema fields and same-store subquery operands. workflow_runtime actions can filter virtual action fields including actionKey, correlation.<key>, and context.<key>. system sources can filter their exposed virtual fields, especially bucketTime/createdAt/connectionId/eventType/tag. Note that filter bindings ({filter,path}) and subquery operands only exist in the flat form — they resolve at query time and have no ConditionExpression equivalent.",
