    ...
    let mut result: Value = serde_json::from_str(r###"{
        ...
        "blockShape": {
            ...
            "table": {
                ...
                "note": "Tables support source.mode='filter' for row data and source.mode='aggregate' for grouped aggregate result sets. Configure visible/searchable/sortable fields in table.columns. A table column may use maxChars for display-only text cutoff, type='chart' for inline aggregate charts, type='value' with source.select for scalar joined lookups, type='workflow_button' with workflowAction for a row-scoped workflow launcher, type='interaction_buttons' with interactionButtons for row-scoped report navigation/action buttons, or table.actions[] for table-wide selected-row workflow launchers. To enable inline writeback on a column, see writeback.editable."
            },
            "chart": {
                "type": "chart",
                "configKey": "chart",
