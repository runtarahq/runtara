...
#[test]
fn production_compile_wrapper_uses_standard_composition_while_legacy_fixture_stays_readable() {
    ...
    let graph = super::super::wasm_performance_baseline::random_chain(1, false);
    let mut reference =
        compile_direct_workflow(input(graph.clone(), &dir.path().join("direct"))).unwrap();
    compose_direct_workflow(&mut reference, &components).unwrap();
    ...
    .unwrap();
    assert!(
        parse(&fs::read(legacy.wasm_path).unwrap(), limits())
            .unwrap()
            .is_some()
    );
    ...
    let wrapped = compile_workflow_direct(
        ...
        DirectWorkflowCompileOptions {
            output_dir: dir.path().join("wrapper"),
    ...
    let wit_component::DecodedWasm::Component(resolve, world) =
        wit_component::decode(&bytes).unwrap()
    else {
        panic!("expected a normally composed component");
    };
