...
fn emit_components_dispatch(agents: &[String]) -> TokenStream {
    ...
    let arms: Vec<TokenStream> = agents
        .iter()
        .map(|agent_id| {
            let agent_module = agent_module_ident(agent_id);
            ...
            quote! {
                #agent_str => {
                    ...
                    let result = bindings::#agent_module::invoke(
                        capability_id,
                        &bytes,
                        conn.as_ref(),
                    );
...
fn emit_cargo_toml(agents: &[String]) -> String {
    // `runtara:agent-<id>` package name per agent — used by wac to look up
    // each component in the CAS.
    let _agents_for_doc: Vec<&str> = agents.iter().map(String::as_str).collect();
    format!(
...
runtara-workflow-stdlib = {{ path = "{stdlib_path}", default-features = false, features = ["wasm"] }}
...
fn emit_world_wit(agents: &[String]) -> String {
    ...
    for agent in agents {
        // Hyphens are valid in WIT identifiers, so the kebab agent id is
        // a legal import name. cargo-component snake-cases it on the Rust
        // side (matches `agent_module_ident` above).
        out.push_str(&format!(
            "    import {}: runtara:agent/capabilities@0.3.0;\n",
            agent
        ));
    }
    out.push_str("    include wasi:cli/command@0.2.0;\n");
    out.push_str("}\n");
    out
}
...
mod tests {
    ...
    fn world_wit_imports_each_agent() {
        ...
        assert!(world.contains("import crypto: runtara:agent/capabilities@0.3.0;"));
        assert!(world.contains("import object-model: runtara:agent/capabilities@0.3.0;"));
