...
use wasm_encoder::{
    ...
    ImportSection, Instruction, MemArg, MemorySection, MemoryType, Module, Section, TypeSection,
    ValType,
};
...
use abi::{
    emit_retptr_error_or_return, load_retptr_list, load_retptr_option_list, load_retptr_tag,
    push_core_type, push_retptr_arg, push_retptr_i64_load, push_retptr_u8_load, push_segment_args,
    return_if_retptr_error, zero_return_function,
};
use agent::emit_agent_plan;
...
use split::{emit_split_append_error_payload_and_continue, emit_split_plan};
use step_context::emit_step_context_plan;
use switch_route::emit_switch_route_plan;
use while_loop::emit_while_plan;
