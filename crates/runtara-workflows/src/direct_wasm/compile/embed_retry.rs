...
use super::{
    ...
    DIRECT_EMBED_RETRY_SLEEP_KEY_LEN_LOCAL, DIRECT_EMBED_RETRY_SLEEP_KEY_PTR_LOCAL,
...
pub(super) fn emit_embed_retry_condition(
    body: &mut WasmFunction,
    max_retries: u32,
    retry_delay_ms: u64,
) {
    body.instruction(&Instruction::LocalGet(DIRECT_EMBED_RETRYABLE_LOCAL));
    body.instruction(&Instruction::If(BlockType::Result(ValType::I32)));
    body.instruction(&Instruction::LocalGet(DIRECT_EMBED_RATE_LIMITED_LOCAL));
    body.instruction(&Instruction::If(BlockType::Result(ValType::I32)));
