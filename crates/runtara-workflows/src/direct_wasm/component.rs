...
pub fn emit_direct_component_artifacts_with_pools(
    ...
    let shared_components = DIRECT_SHARED_COMPONENT_REQUIREMENTS
        ...
        .collect();
    DirectComponentArtifacts {
        world_wit: emit_world_wit(agents, abi, omit_runtime, export_agent_id, parallel_pools),
        wac_source: emit_wac(agents, runtime_binding, parallel_pools),
    ...
    for agent in agents {
        ...
        if let Some(pool) = parallel_pools.get(agent) {
            for member in 1..*pool {
                ...
                out.push_str(&format!(
                    "    import runtara:agent-{phantom}/capabilities@{DIRECT_AGENT_WIT_VERSION};\n"
                ));
            }
        }
    }
        ...
        if let Some(pool) = parallel_pools.get(agent) {
            for member in 1..*pool {
                let phantom =
                    crate::direct_wasm::split_parallel_pool_member_component_id(agent, member);
                out.push_str(&format!(
                    " \"runtara:agent-{phantom}/capabilities@{DIRECT_AGENT_WIT_VERSION}\": \
                     agent-{phantom}[\"runtara:agent-{id}/capabilities@{DIRECT_AGENT_WIT_VERSION}\"],",
                    id = agent
                ));
            }
        }
