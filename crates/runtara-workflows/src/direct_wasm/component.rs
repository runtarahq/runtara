...
pub fn emit_direct_component_artifacts_with_binding(
    ...
    emit_direct_component_artifacts_configured(agents, runtime_binding, WorkflowAbi::default())
}
...
pub fn emit_direct_component_artifacts_configured(
    ...
    let shared_components = DIRECT_SHARED_COMPONENT_REQUIREMENTS
        .iter()
        .filter(|component| {
            runtime_binding == RuntimeBinding::Composed
                || component.package != "runtara:workflow-runtime"
        })
        .copied()
        .collect();
    DirectComponentArtifacts {
        world_wit: emit_world_wit(agents, abi),
...
fn emit_world_wit(agents: &[String], abi: WorkflowAbi) -> String {
    let mut out = format!(
        "// Generated by runtara-workflows direct component scaffold.\n\
         ...
         \x20   import runtara:workflow-runtime/runtime@{WORKFLOW_WIT_VERSION};\n",
    );
    for agent in agents {
        out.push_str(&format!(
