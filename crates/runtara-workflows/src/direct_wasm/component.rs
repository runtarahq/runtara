...
/// Package id a workflow-as-agent exports its capabilities under. Fixed for
/// slice a (the ABI itself); slice b parameterizes it to the workflow's own
/// agent id so composed children get distinct packages.
...
fn emit_world_wit(agents: &[String], abi: WorkflowAbi, omit_runtime: bool) -> String {
    ...
    match abi {
        ...
        WorkflowAbi::AgentCapabilities => out.push_str(&format!(
            "    export runtara:agent-{CAPABILITIES_EXPORT_AGENT_ID}/capabilities@{DIRECT_AGENT_WIT_VERSION};\n"
        )),
    }
    out.push_str("}\n");
    out
