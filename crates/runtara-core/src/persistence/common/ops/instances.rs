...
//! - `complete_instance_extended` and the `_with_termination` variants
//!   now set `finished_at` only when the new status is terminal, matching
//!   Postgres' `CASE WHEN status IN (...) THEN NOW() ELSE finished_at END`.
//!   Previously SQLite set `finished_at` unconditionally on every call.
//! - The same calls now apply `COALESCE($stderr, stderr)` instead of
...
macro_rules! impl_instance_ops {
    ($Backend:ty, $Pool:ty, $Dialect:ty) => {
        impl $Backend {
            ...
            pub(crate) async fn op_complete_instance(
                pool: &$Pool,
                instance_id: &str,
                output: ::core::option::Option<&[u8]>,
                error: ::core::option::Option<&str>,
            ) -> ::core::result::Result<(), $crate::error::CoreError> {
                use $crate::persistence::common::error::not_found_if_empty;
                use $crate::persistence::dialect::{Dialect, EnumKind};
                let p1 = <$Dialect>::placeholder(1);
            ...
            /// UPDATE with extended fields (stderr COALESCE, checkpoint
            /// COALESCE) and conditional `finished_at` via a CASE that only
            /// fires on terminal statuses.
            pub(crate) async fn op_complete_instance_extended(
                pool: &$Pool,
                instance_id: &str,
                status: &str,
                output: ::core::option::Option<&[u8]>,
                error: ::core::option::Option<&str>,
                stderr: ::core::option::Option<&str>,
                ...
                let sql = format!(
                    "UPDATE instances \
                     SET status = {p2}{status_cast}, \
                         output = {p3}, \
                         error = {p4}, \
                         stderr = COALESCE({p5}, stderr), \
                         checkpoint_id = COALESCE({p6}, checkpoint_id), \
                         finished_at = CASE \
            ...
            /// Same as `op_complete_instance_extended` but only fires when
            /// `status = 'running'`. Returns `true` iff the update matched.
            #[allow(clippy::too_many_arguments)]
            pub(crate) async fn op_complete_instance_if_running(
                pool: &$Pool,
                instance_id: &str,
                status: &str,
                output: ::core::option::Option<&[u8]>,
                ...
                use $crate::persistence::common::error::RowsAffected;
                use $crate::persistence::dialect::{Dialect, EnumKind};
                let p1 = <$Dialect>::placeholder(1);
                let p2 = <$Dialect>::placeholder(2);
                let p3 = <$Dialect>::placeholder(3);
                    ...
                    "UPDATE instances \
                     SET status = {p2}{status_cast}, \
                         ...
                         finished_at = CASE \
                             ...
                             ELSE finished_at \
                         END \
                     WHERE instance_id = {p1} AND status = 'running'"
                );
                let result = ::sqlx::query(&sql)
                    .bind(instance_id)
                    .bind(status)
                    ...
                    .execute(pool)
                    .await?;
                Ok(result.rows_affected_generic() > 0)
            }

            /// UPDATE with termination tracking (`termination_reason`,
            /// `exit_code`) and the same conditional-`finished_at` CASE as
            /// `op_complete_instance_extended`. Returns unit; does *not*
            /// require the previous status to be `running`.
            #[allow(clippy::too_many_arguments)]
            pub(crate) async fn op_complete_instance_with_termination(
                pool: &$Pool,
                instance_id: &str,
                status: &str,
                termination_reason: ::core::option::Option<&str>,
                exit_code: ::core::option::Option<i32>,
                output: ::core::option::Option<&[u8]>,
                error: ::core::option::Option<&str>,
                stderr: ::core::option::Option<&str>,
                checkpoint_id: ::core::option::Option<&str>,
            ...
            /// Same as `op_complete_instance_with_termination` but only
            /// fires when the current status is `running`. Returns `true`
            /// iff the update matched.
            #[allow(clippy::too_many_arguments)]
            pub(crate) async fn op_complete_instance_with_termination_if_running(
                pool: &$Pool,
                instance_id: &str,
                status: &str,
                termination_reason: ::core::option::Option<&str>,
