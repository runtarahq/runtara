...
pub enum ColumnType {
    ...
    /// same schema. Stored via `GENERATED ALWAYS AS (to_tsvector('<lang>',
    /// "<source>")) STORED`. Read-only from the user's perspective —
    /// `validate_value` always rejects writes; the column never appears in
    /// CSV import/export and is omitted from row payloads (clients reach it
    /// via `MATCH` / `TS_RANK`).
    Tsvector {
...
impl ColumnType {
    ...
    /// True if this column's storage is generated by the database (e.g. a
    /// `tsvector` column derived from another column). Generated columns are
    /// skipped on insert/update bind paths and on CSV import/export.
    pub fn is_generated(&self) -> bool {
        matches!(self, ColumnType::Tsvector { .. })
    }
