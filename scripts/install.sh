...
write_config() {
    ...
    # refuses to start otherwise. Default the host accordingly and skip OAUTH2_* vars.
    local server_host auth_lines
...
AUTHEOF
        ...
        local)
            ...
            auth_lines=$(cat <<AUTHEOF
AUTH_PROVIDER=local
# No OAUTH2_* settings needed in local mode. The server refuses to start unless
# SERVER_HOST is a loopback address (127.0.0.1, ::1, or localhost).
AUTHEOF
)
            ;;
...
AUTHEOF
            ...
            ;;
        *)
            die 1 "Unknown AUTH_PROVIDER value: '${auth_provider}'. Must be one of: oidc, local, trust_proxy"
            ;;
    esac
