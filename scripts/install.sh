...
#
#   # Docker simple mode (Postgres + Valkey + Runtara in containers):
#   curl -fsSL https://install.runtara.com | sh -s -- --docker
#   curl -fsSL https://install.runtara.com | sh -s -- --docker --version 1.6.10
#   curl -fsSL https://install.runtara.com | sh -s -- --docker-persist          # keep data across restarts
...
do_docker_install() {
    step "Docker simple mode"

    # Check for docker
    if ! command -v docker > /dev/null 2>&1; then
        die $EXIT_MISSING_DEPS "docker not found. Install Docker first: https://docs.docker.com/get-docker/"
    fi

    # Check docker compose plugin
    if ! docker compose version > /dev/null 2>&1; then
        die $EXIT_MISSING_DEPS "docker compose plugin not found. Install Docker Compose: https://docs.docker.com/compose/install/"
    fi

    # Resolve version for the image tag
    local version="${RUNTARA_VERSION:-}"
    if [ -z "$version" ]; then
    ...
    info "Runtara version: ${version}"

    # Determine install directory
    local dir="${DOCKER_DIR:-${HOME}/.runtara-docker}"
    mkdir -p "$dir"
    ...
    local vk_port="${VALKEY_PORT:-6379}"
    local tenant_id="${TENANT_ID:-default}"

    # Volume config
    local pg_volume="runtara-pgdata:/var/lib/postgresql/data"
    local vk_volume="runtara-valkey:/data"
    local volume_block=""
...
volumes:
    ...
    # Write init SQL to create the objects database
    cat > "${dir}/init-db.sql" <<'SQLEOF'
CREATE DATABASE runtara_objects OWNER runtara;
SQLEOF

    # Write Dockerfile that installs runtara from the GitHub release
...
# Runtara simple mode — generated by install.sh --docker
